  ...
  <style>
    ...
    *::after {
    ...
    .studio-main {
      ...
      max-height: calc(100vh - 46px - 128px); /* topbar=46 + ctrlbar=128 */
    }
...
.studio-ctrl-v2 {
  ...
  min-height: 120px;    /* always show both rows */
  max-height: 132px;    /* cap so cam gets rest */
  overflow: hidden;
}
.studio-ctrl-row {
  display: flex;
...
/* ── STUDIO BOTTOM BAR COMPACT ── */
.studio-ctrl-v2 { max-height: 132px !important; }
.studio-ctrl-row { max-height: 64px !important; overflow-y: hidden; }
.panel-group { flex-shrink: 0; min-width: 0; }
.panel-group-body { padding: 3px 8px !important; gap: 4px !important; }
.panel-group-label { padding: 2px 8px !important; }
.grp-bg .panel-group-body { max-width: 280px; flex-wrap: wrap; }
.s-btn { font-size: 9px !important; padding: 3px 6px !important; }

/* ── STUDIO LAYOUT FIX — camera never covers bottom panel ── */
.studio-wrap { height: 100vh !important; display: flex !important; flex-direction: column !important; overflow: hidden !important; }
.studio-bar { flex-shrink: 0 !important; height: 46px !important; }
.studio-main { flex: 1 !important; min-height: 0 !important; overflow: hidden !important; max-height: calc(100vh - 46px - 128px) !important; }
.studio-ctrl-v2 { flex-shrink: 0 !important; min-height: 120px !important; max-height: 132px !important; overflow: hidden !important; }
.studio-ctrl-row { max-height: 64px !important; overflow-y: hidden !important; }
.panel-group { flex-shrink: 0 !important; }
.panel-group-body { padding: 3px 8px !important; gap: 4px !important; }
.panel-group-label { padding: 2px 8px !important; }
.grp-bg .panel-group-body { max-width: 280px !important; flex-wrap: wrap !important; }
.s-btn { font-size: 9px !important; padding: 3px 6px !important; }
</style>
<style id="tp-smooth-css">
/* Teleprompter smooth scroll */
#studio-tp, #tp-wrap { position:relative; overflow:hidden !important; }
#tp-cur {
  will-change:transform;
  ...
  transition:none !important;
    ...
    <style>
    /* Professional track color system */
    .tl-track[data-type="video"]   { --track-color: #3b82f6; }  /* Blue = main video */
    .tl-track[data-type="voice"]   { --track-color: #8b5cf6; }  /* Purple = voice */
    .tl-track[data-type="music"]   { --track-color: #10b981; }  /* Green = music */
    .tl-track[data-type="sfx"]     { --track-color: #10b981; }  /* Green = SFX */
    .tl-track[data-type="image"]   { --track-color: #f59e0b; }  /* Orange = images */
    .tl-track[data-type="effects"] { --track-color: #00d4ff; }  /* Cyan = effects */
    .tl-track[data-type="text"]    { --track-color: #eab308; }  /* Yellow = text */
    .tl-clip { background: var(--track-color, rgba(0,212,255,.25)) !important; border-color: var(--track-color, rgba(0,212,255,.4)) !important; }
    /* Timeline resize cursor */
    #tl-resize-handle:hover { background: rgba(0,212,255,.2) !important; }
    /* Studio ctrl - ensure rows wrap properly not cut off */
    .studio-ctrl-v2 { overflow: visible !important; }
    .studio-ctrl-row { flex-wrap: wrap; overflow: visible; }
    /* TP body - ensure visible */
    .tp-body { display: flex !important; flex-direction: column; min-height: 80px; }
    #tp-cur:empty::after { content: 'Open a project with scenes to see your script here'; color: rgba(255,255,255,.2); font-size: 14px; }
    </style>
<body>
  <!-- TOASTS -->
  <div id="toasts"></div>
  ...
  <nav id="topnav" style="display:none;">
    ...
    <div class="nav-links" id="nav-app-links" style="display:none;"></div>
    <div class="nav-right">
  ...
  <div id="home" class="pg on" style="background:var(--navy);color:#fff;">
    ...
    <div id="home-nav"
      ...
      <div style="margin-left:auto;display:flex;align-items:center;gap:8px;">
        <!-- Logged OUT state -->
        <button class="btn btn-ghost btn-sm" id="home-signin-btn" onclick="nav('auth-pg')">Sign in</button>
        <button class="btn btn-primary btn-sm" id="home-start-btn" onclick="nav('auth-pg')">Get started free</button>
        <!-- Logged IN state (hidden until auth) -->
        <button class="btn btn-ghost btn-sm" id="home-dashboard-btn" style="display:none;" onclick="nav('dashboard-pg')">Dashboard</button>
        <button class="btn btn-primary btn-sm" id="home-studio-btn" style="display:none;" onclick="heroStart()">Open Studio</button>
      </div>
    </div>
    ...
    <div class="dash-layout">
      <div class="dash-sidebar">
        ...
        <div class="sb-nav">
          ...
          <button class="sb-item" onclick="dashNav('my-videos',this)">
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <rect x="1" y="2" width="12" height="10" rx="2" stroke="currentColor" stroke-width="1.2"/>
              <path d="M6 5l4 2-4 2V5z" fill="currentColor"/>
            </svg>My Videos
          </button>
          <button class="sb-item" onclick="dashNav('my-assets',this)">
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <rect x="1" y="1" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/>
              <rect x="7" y="1" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/>
              <rect x="1" y="7" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/>
              <rect x="7" y="7" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/>
            </svg>My Assets
          </button>
          <div class="sb-div"></div>
      ...
      <div class="dash-main" style="flex:1;overflow-y:auto;padding:24px;">
        ...
        <!-- MY VIDEOS -->
        <div class="dash-sec" id="dash-my-videos">
          <div class="dash-hdr">
            <div>
              <div class="dash-title">My Videos</div>
              <div class="dash-sub">All your recorded and AI-generated clips</div>
            </div>
            <div style="display:flex;gap:8px;">
              <button class="btn btn-ghost btn-sm" onclick="dash_loadVideos()">↻ Refresh</button>
              <button class="btn btn-primary btn-sm" onclick="nav('edit-pg')">Open Edit Suite →</button>
            </div>
          </div>
          <!-- Filter tabs -->
          <div style="display:flex;gap:6px;margin-bottom:16px;">
            <button class="btn btn-ghost btn-sm dash-vid-filter on" data-filter="all" onclick="dash_filterVideos('all',this)">All</button>
            <button class="btn btn-ghost btn-sm dash-vid-filter" data-filter="recording" onclick="dash_filterVideos('recording',this)">🎥 Recorded</button>
            <button class="btn btn-ghost btn-sm dash-vid-filter" data-filter="ai-presenter" onclick="dash_filterVideos('ai-presenter',this)">✨ AI Generated</button>
          </div>
          <div id="dash-videos-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;">
            <div style="color:rgba(255,255,255,.3);font-size:13px;grid-column:1/-1;padding:24px 0;text-align:center;">
              Loading your videos...
            </div>
          </div>
        </div>
        ...
        <!-- MY ASSETS -->
        <div class="dash-sec" id="dash-my-assets">
          <div class="dash-hdr">
            <div>
              <div class="dash-title">My Assets</div>
              <div class="dash-sub">Images, audio, video and SFX saved to your library</div>
            </div>
          </div>
          <!-- Upload buttons -->
          <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;">
            <label class="btn btn-ghost btn-sm" style="cursor:pointer;">
              🖼 Upload Image<input type="file" style="display:none;" accept="image/*" onchange="dash_uploadAsset(this.files[0],'image')">
            </label>
            <label class="btn btn-ghost btn-sm" style="cursor:pointer;">
              🎵 Upload Audio<input type="file" style="display:none;" accept="audio/*" onchange="dash_uploadAsset(this.files[0],'audio')">
            </label>
            <label class="btn btn-ghost btn-sm" style="cursor:pointer;">
              🎬 Upload Video<input type="file" style="display:none;" accept="video/*" onchange="dash_uploadAsset(this.files[0],'video')">
            </label>
            <label class="btn btn-ghost btn-sm" style="cursor:pointer;">
              🎤 Upload SFX<input type="file" style="display:none;" accept="audio/*" onchange="dash_uploadAsset(this.files[0],'sfx')">
            </label>
          </div>
          <!-- Filter by type -->
          <div style="display:flex;gap:6px;margin-bottom:14px;">
            <button class="btn btn-ghost btn-sm dash-asset-filter on" data-type="all" onclick="dash_filterAssets('all',this)">All</button>
            <button class="btn btn-ghost btn-sm dash-asset-filter" data-type="image" onclick="dash_filterAssets('image',this)">🖼 Images</button>
            <button class="btn btn-ghost btn-sm dash-asset-filter" data-type="audio" onclick="dash_filterAssets('audio',this)">🎵 Audio</button>
            <button class="btn btn-ghost btn-sm dash-asset-filter" data-type="video" onclick="dash_filterAssets('video',this)">🎬 Video</button>
            <button class="btn btn-ghost btn-sm dash-asset-filter" data-type="sfx" onclick="dash_filterAssets('sfx',this)">🎤 SFX</button>
          </div>
          <div id="dash-assets-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px;">
            <div style="color:rgba(255,255,255,.3);font-size:13px;grid-column:1/-1;padding:24px 0;text-align:center;">
              No assets yet — upload files above
            </div>
          </div>
        </div>
        ...
        <div class="scenes-sidebar">
          <div class="sb-hdr">
            ...
            <div class="sb-tabs">
              ...
              <button class="sb-tab" onclick="sidebarTab('my-assets',this)">My Assets</button>
          ...
          <div class="sb-body">
            ...
            <!-- MY ASSETS TAB — images, audio, video, SFX, generated AI clips -->
            <div id="my-assets-panel" style="display:none;">
              <div style="padding:8px 0 4px;font-size:9px;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.05em;">Upload to Asset Library</div>
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px;">
                <label style="background:rgba(255,255,255,.06);border:.5px solid rgba(255,255,255,.12);border-radius:8px;padding:8px;text-align:center;cursor:pointer;font-size:10px;color:rgba(255,255,255,.6);">
                  🖼 Image<input type="file" style="display:none;" accept="image/*" onchange="asset_upload(this.files[0],'image')">
                </label>
                <label style="background:rgba(255,255,255,.06);border:.5px solid rgba(255,255,255,.12);border-radius:8px;padding:8px;text-align:center;cursor:pointer;font-size:10px;color:rgba(255,255,255,.6);">
                  🎵 Audio<input type="file" style="display:none;" accept="audio/*" onchange="asset_upload(this.files[0],'audio')">
                </label>
                <label style="background:rgba(255,255,255,.06);border:.5px solid rgba(255,255,255,.12);border-radius:8px;padding:8px;text-align:center;cursor:pointer;font-size:10px;color:rgba(255,255,255,.6);">
                  🎬 Video<input type="file" style="display:none;" accept="video/*" onchange="asset_upload(this.files[0],'video')">
                </label>
                <label style="background:rgba(255,255,255,.06);border:.5px solid rgba(255,255,255,.12);border-radius:8px;padding:8px;text-align:center;cursor:pointer;font-size:10px;color:rgba(255,255,255,.6);">
                  🎤 SFX<input type="file" style="display:none;" accept="audio/*" onchange="asset_upload(this.files[0],'sfx')">
                </label>
              </div>
              <div style="font-size:9px;color:rgba(255,255,255,.3);margin-bottom:6px;text-transform:uppercase;letter-spacing:.05em;">Saved Assets</div>
              <div id="my-assets-grid"></div>
            </div>
  ...
  <div id="record-pg" class="pg-full" style="background:var(--navy);">
    <div class="studio-wrap" style="height:100vh;display:flex;flex-direction:column;overflow:hidden;">
      ...
      <div class="studio-main" style="flex:1;min-height:0;overflow:hidden;">
        <!-- Scene queue -->
        <div class="studio-q">
        ...
        <div class="studio-canvas" id="studio-canvas">
          ...
          <div class="studio-tp" id="studio-tp">
            <!-- Collapse handle -->
            <div class="tp-handle" id="tp-drag-handle" style="cursor:move;display:flex;align-items:center;gap:6px;" title="Click label to expand/collapse">
              <span style="font-size:11px;color:rgba(255,255,255,.15);margin-right:2px;">⠿</span>
              <span class="tp-handle-label" onclick="toggleTpPanel()" style="cursor:pointer;font-size:11px;font-weight:600;color:rgba(255,255,255,.5);letter-spacing:.04em;">TELEPROMPTER</span>
              <span id="tp-collapse-icon" onclick="toggleTpPanel()" style="cursor:pointer;font-size:10px;color:rgba(255,255,255,.3);margin-left:2px;" title="Expand/Collapse">▲</span>
              <div style="margin-left:auto;display:flex;align-items:center;gap:8px;">
      ...
      <div class="studio-ctrl-v2" style="position:relative;z-index:50;flex-shrink:0;min-height:128px;overflow:visible;">
      ...
      <select id="tp-mode-sel" onchange="window._tpMode=this.value" title="Scroll mode" style="font-size:9px;background:rgba(255,255,255,.06);border:.5px solid rgba(255,255,255,.1);color:rgba(255,255,255,.5);border-radius:4px;padding:2px 4px;cursor:pointer;"><option value="scroll" selected>📜 Scroll</option><option value="word">💡 Word</option><option value="line">📄 Line</option></select>
      </div>
      ...
      <!-- SESSION -->
      <!-- SESSION — recording info + saved count -->
      <div class="panel-group grp-session" style="margin-left:auto;">
      <div class="panel-group-label">✅ SESSION</div>
      <div class="panel-group-body" style="gap:6px;align-items:center;padding:4px 8px;">
        <div id="st-rec-time" style="font-size:16px;font-weight:700;color:#10b981;font-family:'JetBrains Mono',monospace;">0:00</div>
        <div style="font-size:9px;color:rgba(255,255,255,.35);padding-left:6px;border-left:.5px solid rgba(255,255,255,.08);">
          <span style="color:#10b981;font-weight:600;" id="st-clips-saved">0</span><br><span style="color:rgba(255,255,255,.2);">saved</span>
        </div>
      </div>
      </div>
      ...
      <div class="panel-group-label">🎬 Background</div>
      <div class="panel-group-body" style="gap:3px;flex-wrap:wrap;max-width:280px;">
      <button class="bg-btn s-btn on" data-bg="none" onclick="setBg(this)" style="font-size:9px;padding:3px 6px;">Live</button>
      <button class="bg-btn s-btn" data-bg="blur" onclick="setBg(this)" style="font-size:9px;padding:3px 6px;">Blur</button>
      <button class="bg-btn s-btn" data-bg="dark" onclick="setBg(this)" style="font-size:9px;padding:3px 6px;">Dark</button>
      <button class="bg-btn s-btn" data-bg="news" onclick="setBg(this)" style="font-size:9px;padding:3px 6px;">📺 News</button>
      <button class="bg-btn s-btn" data-bg="office" onclick="setBg(this)" style="font-size:9px;padding:3px 6px;">💼 Office</button>
      <button class="bg-btn s-btn" data-bg="library" onclick="setBg(this)" style="font-size:9px;padding:3px 6px;">📚 Library</button>
      <button class="bg-btn s-btn" data-bg="green" onclick="setBg(this)" style="font-size:9px;padding:3px 6px;">🟩 Green</button>
  ...
  <!-- ══ EDIT SUITE ═══════════════════════════════════════════════════════════════ -->
  <div id="edit-pg" class="pg-full" style="display:none;flex-direction:column;height:100vh;overflow:hidden;background:#070910;">
    
    <!-- ═══ TOPBAR ═══════════════════════════════════════════════════════ -->
    <div id="edit-topbar" style="flex-shrink:0;height:44px;display:flex;align-items:center;gap:0;background:#07090f;border-bottom:.5px solid rgba(255,255,255,.06);z-index:100;">
      <!-- Back + title -->
      <div style="display:flex;align-items:center;gap:8px;padding:0 14px;border-right:.5px solid rgba(255,255,255,.06);height:100%;">
        <button onclick="nav('dashboard-pg')" style="background:none;border:none;color:rgba(255,255,255,.3);font-size:11px;cursor:pointer;padding:0;display:flex;align-items:center;gap:4px;">← Dashboard</button>
        <div style="width:.5px;height:14px;background:rgba(255,255,255,.08);"></div>
        <span style="font-size:12px;font-weight:700;color:rgba(255,255,255,.9);" id="edit-title">✂️ Edit Suite</span>
        <span id="edit-save-status" style="font-size:9px;color:rgba(255,255,255,.2);margin-left:4px;">Saved</span>
      </div>
      <!-- Tool groups -->
      <div style="display:flex;align-items:center;height:100%;padding:0 8px;gap:2px;border-right:.5px solid rgba(255,255,255,.06);">
        <button class="edit-tool-btn" onclick="tl_setTool('select')" id="tl-btn-select" title="Select (V)" style="background:rgba(0,212,255,.15);color:#00d4ff;border:.5px solid rgba(0,212,255,.3);border-radius:5px;padding:4px 8px;font-size:10px;cursor:pointer;">↖ Select</button>
        <button class="edit-tool-btn" onclick="tl_setTool('trim')" id="tl-btn-trim" title="Trim (T)" style="background:none;border:.5px solid rgba(255,255,255,.1);border-radius:5px;padding:4px 8px;font-size:10px;cursor:pointer;color:rgba(255,255,255,.5);">↔ Trim</button>
        <button class="edit-tool-btn" onclick="tl_split()" id="tl-btn-split" title="Split (S)" style="background:none;border:.5px solid rgba(255,255,255,.1);border-radius:5px;padding:4px 8px;font-size:10px;cursor:pointer;color:rgba(255,255,255,.5);">⊣ Split</button>
        <button class="edit-tool-btn" onclick="tl_setTool('razor')" id="tl-btn-razor" title="Razor (R)" style="background:none;border:.5px solid rgba(255,255,255,.1);border-radius:5px;padding:4px 8px;font-size:10px;cursor:pointer;color:rgba(255,255,255,.5);">✂ Razor</button>
      </div>
      <!-- Import -->
      <div style="display:flex;align-items:center;height:100%;padding:0 8px;gap:2px;border-right:.5px solid rgba(255,255,255,.06);">
        <button onclick="importMedia('video')" style="background:none;border:.5px solid rgba(255,255,255,.1);border-radius:5px;padding:4px 8px;font-size:10px;cursor:pointer;color:rgba(255,255,255,.4);">+ Video</button>
        <button onclick="importMedia('audio')" style="background:none;border:.5px solid rgba(255,255,255,.1);border-radius:5px;padding:4px 8px;font-size:10px;cursor:pointer;color:rgba(255,255,255,.4);">+ Audio</button>
        <button onclick="importMedia('image')" style="background:none;border:.5px solid rgba(255,255,255,.1);border-radius:5px;padding:4px 8px;font-size:10px;cursor:pointer;color:rgba(255,255,255,.4);">+ Image</button>
      </div>
      <!-- AI Presenter link -->
      <div style="display:flex;align-items:center;height:100%;padding:0 8px;border-right:.5px solid rgba(255,255,255,.06);">
        <button onclick="openAIPresenterStudio()" style="background:rgba(139,92,246,.12);border:.5px solid rgba(139,92,246,.3);border-radius:5px;padding:4px 10px;font-size:10px;cursor:pointer;color:rgba(139,92,246,.9);">🤖 AI Presenter</button>
      </div>
      <!-- Right: export -->
      <div style="margin-left:auto;display:flex;align-items:center;gap:8px;padding:0 14px;height:100%;">
        <span id="tl-duration-display" style="font-size:9px;font-family:'JetBrains Mono',monospace;color:rgba(255,255,255,.2);">0:00</span>
        <button onclick="nav('export-pg')" style="background:linear-gradient(135deg,rgba(0,212,255,.2),rgba(139,92,246,.2));border:.5px solid rgba(0,212,255,.3);border-radius:6px;padding:5px 14px;font-size:11px;font-weight:600;color:#00d4ff;cursor:pointer;">Export →</button>
      </div>
    </div>

    <!-- ═══ MAIN BODY (preview + inspector + clips) ═════════════════════ -->
    <div style="flex:1;display:flex;overflow:hidden;min-height:0;">

      <!-- LEFT: Clips / Media bin -->
      <div id="edit-media-bin" style="width:200px;flex-shrink:0;display:flex;flex-direction:column;background:#050810;border-right:.5px solid rgba(255,255,255,.06);overflow:hidden;">
        <div style="flex-shrink:0;padding:8px 10px;border-bottom:.5px solid rgba(255,255,255,.05);font-size:9px;font-weight:700;color:rgba(255,255,255,.3);letter-spacing:.08em;">MEDIA BIN</div>
        <div id="edit-clips-strip-inner" style="flex:1;overflow-y:auto;padding:6px;display:flex;flex-direction:column;gap:4px;">
          <div style="font-size:10px;color:rgba(255,255,255,.2);text-align:center;padding:20px 0;">No clips yet</div>
        </div>
        <div style="flex-shrink:0;border-top:.5px solid rgba(255,255,255,.05);padding:6px;">
          <div id="edit-clip-count" style="font-size:9px;color:rgba(255,255,255,.2);text-align:center;"></div>
        </div>
      </div>

      <!-- CENTER: Preview monitor -->
      <div id="edit-preview-pane" style="flex:1;display:flex;flex-direction:column;background:#050810;min-width:0;overflow:hidden;">
        <!-- Video area -->
        <div style="flex:1;display:flex;align-items:center;justify-content:center;position:relative;background:#080b16;overflow:hidden;">
          <div id="edit-no-clips" style="text-align:center;color:rgba(255,255,255,.2);">
            <div style="font-size:32px;margin-bottom:10px;">🎬</div>
            <div style="font-size:13px;margin-bottom:8px;">No clips yet</div>
            <div style="font-size:10px;color:rgba(255,255,255,.12);">Record scenes or generate with AI Presenter</div>
          </div>
          <video id="edit-video" preload="auto"
            style="display:none;max-width:100%;max-height:100%;object-fit:contain;"
            ontimeupdate="edit_onTimeUpdate()"
            onended="edit_onEnded()"
            onloadedmetadata="edit_onMetadata()"></video>
          <!-- Subtitle overlay -->
          <div id="edit-subtitle-overlay" style="position:absolute;bottom:40px;left:8%;right:8%;text-align:center;font-size:18px;font-weight:600;color:#fff;text-shadow:0 2px 4px rgba(0,0,0,.8);pointer-events:none;display:none;"></div>
          <!-- Scene info badge -->
          <div id="edit-scene-info" style="position:absolute;top:8px;left:8px;background:rgba(0,0,0,.6);padding:3px 7px;border-radius:5px;font-size:9px;color:rgba(255,255,255,.5);display:none;"></div>
          <!-- Safe zones overlay -->
          <div id="edit-safe-zones" style="position:absolute;inset:0;pointer-events:none;display:none;">
            <div style="position:absolute;inset:5%;border:.5px solid rgba(255,255,255,.1);border-radius:2px;"></div>
            <div style="position:absolute;inset:10%;border:.5px solid rgba(255,255,255,.06);border-radius:2px;"></div>
          </div>
        </div>
        <!-- Transport controls -->
        <div style="flex-shrink:0;padding:6px 12px;background:rgba(0,0,0,.5);display:flex;align-items:center;gap:6px;border-top:.5px solid rgba(255,255,255,.06);">
          <button onclick="edit_skipTo(0)" title="Go to start" style="background:none;border:none;color:rgba(255,255,255,.4);font-size:12px;cursor:pointer;padding:2px 5px;">⏮</button>
          <button onclick="editPlayPause()" id="edit-play-btn" style="background:rgba(0,212,255,.15);border:.5px solid rgba(0,212,255,.3);color:#00d4ff;border-radius:6px;padding:5px 14px;font-size:12px;cursor:pointer;">▶</button>
          <button onclick="edit_skipTo(999999)" title="Go to end" style="background:none;border:none;color:rgba(255,255,255,.4);font-size:12px;cursor:pointer;padding:2px 5px;">⏭</button>
          <input type="range" id="edit-seek-bar" min="0" max="100" value="0" step="0.1"
            style="flex:1;accent-color:#00d4ff;height:3px;"
            oninput="edit_seekTo(this.value)">
          <span id="edit-time-display" style="font-size:10px;font-family:'JetBrains Mono',monospace;color:rgba(255,255,255,.4);min-width:80px;text-align:right;">0:00 / 0:00</span>
          <div style="display:flex;align-items:center;gap:4px;">
            <span style="font-size:9px;color:rgba(255,255,255,.2);">🔊</span>
            <input type="range" id="edit-volume" min="0" max="1" step="0.05" value="1"
              style="width:60px;accent-color:#00d4ff;"
              oninput="if(document.getElementById('edit-video'))document.getElementById('edit-video').volume=this.value">
          </div>
          <button onclick="toggleSafeZones()" title="Safe zones" style="background:none;border:.5px solid rgba(255,255,255,.08);border-radius:4px;padding:3px 6px;font-size:9px;color:rgba(255,255,255,.3);cursor:pointer;">⊞</button>
          <button onclick="tl_toggleSubtitles()" id="tl-sub-btn" style="background:none;border:.5px solid rgba(255,255,255,.08);border-radius:4px;padding:3px 6px;font-size:9px;color:rgba(255,255,255,.3);cursor:pointer;">CC</button>
        </div>
      </div>

      <!-- RIGHT: Inspector panel -->
      <div id="edit-inspector" style="width:220px;flex-shrink:0;display:flex;flex-direction:column;background:#050810;border-left:.5px solid rgba(255,255,255,.06);overflow-y:auto;">
        <div style="flex-shrink:0;padding:8px 10px;border-bottom:.5px solid rgba(255,255,255,.05);font-size:9px;font-weight:700;color:rgba(255,255,255,.3);letter-spacing:.08em;">INSPECTOR</div>
        <div id="edit-inspector-body" style="flex:1;padding:10px;">
          <div style="font-size:10px;color:rgba(255,255,255,.2);text-align:center;padding:20px 0;">Select a clip to inspect</div>
        </div>
      </div>
    ...
    <div id="tl-resize-handle"
      style="flex-shrink:0;height:5px;background:rgba(255,255,255,.04);cursor:ns-resize;border-top:.5px solid rgba(255,255,255,.06);transition:background .15s;"
      onmouseenter="this.style.background='rgba(0,212,255,.2)'"
      onmouseleave="this.style.background='rgba(255,255,255,.04)'"
      onmousedown="tl_startResize(event)"
      title="Drag to resize timeline"></div>

    <!-- ═══ TIMELINE ════════════════════════════════════════════════════ -->
    <div id="timeline-panel" style="flex-shrink:0;height:260px;min-height:140px;display:flex;flex-direction:column;border-top:.5px solid rgba(255,255,255,.07);background:#04060e;">
      <!-- Timeline toolbar -->
      <div style="flex-shrink:0;height:36px;display:flex;align-items:center;gap:4px;padding:0 8px;background:rgba(0,0,0,.3);border-bottom:.5px solid rgba(255,255,255,.05);">
        <!-- Tool buttons -->
        <div style="display:flex;gap:2px;">
          <button class="tl-tool on" id="tl-tool-select" onclick="tl_setTool('select')" title="Select (V)" style="padding:2px 7px;font-size:9px;border-radius:3px;border:.5px solid rgba(0,212,255,.3);background:rgba(0,212,255,.12);color:#00d4ff;cursor:pointer;">↖</button>
          <button class="tl-tool" id="tl-tool-trim" onclick="tl_setTool('trim')" title="Trim (T)" style="padding:2px 7px;font-size:9px;border-radius:3px;border:.5px solid rgba(255,255,255,.08);background:none;color:rgba(255,255,255,.4);cursor:pointer;">↔</button>
          <button class="tl-tool" id="tl-tool-split" onclick="tl_split()" title="Split (S)" style="padding:2px 7px;font-size:9px;border-radius:3px;border:.5px solid rgba(255,255,255,.08);background:none;color:rgba(255,255,255,.4);cursor:pointer;">⊣</button>
        </div>
        <div style="width:.5px;height:14px;background:rgba(255,255,255,.08);margin:0 4px;"></div>
        <!-- Zoom -->
        <button onclick="tl_zoom(-1)" title="Zoom out" style="padding:2px 6px;font-size:9px;border-radius:3px;border:.5px solid rgba(255,255,255,.08);background:none;color:rgba(255,255,255,.3);cursor:pointer;">−</button>
        <span id="tl-zoom-label" style="font-size:9px;color:rgba(255,255,255,.2);min-width:28px;text-align:center;">1x</span>
        <button onclick="tl_zoom(1)" title="Zoom in" style="padding:2px 6px;font-size:9px;border-radius:3px;border:.5px solid rgba(255,255,255,.08);background:none;color:rgba(255,255,255,.3);cursor:pointer;">+</button>
        <div style="width:.5px;height:14px;background:rgba(255,255,255,.08);margin:0 4px;"></div>
        <!-- Add tracks -->
        <button onclick="tl_addTrack('text')" style="padding:2px 8px;font-size:9px;border-radius:3px;border:.5px solid rgba(234,179,8,.2);background:rgba(234,179,8,.06);color:rgba(234,179,8,.7);cursor:pointer;">+ Text</button>
        <button onclick="tl_addTrack('music')" style="padding:2px 8px;font-size:9px;border-radius:3px;border:.5px solid rgba(16,185,129,.2);background:rgba(16,185,129,.06);color:rgba(16,185,129,.7);cursor:pointer;">+ Music</button>
        <!-- Right -->
      ...
      <div style="flex:1;display:flex;overflow:hidden;min-height:0;">
        <!-- Track labels -->
        <div id="tl-labels" style="width:90px;flex-shrink:0;border-right:.5px solid rgba(255,255,255,.07);overflow:hidden;display:flex;flex-direction:column;">
          <!-- Ruler spacer -->
          <div style="height:20px;border-bottom:.5px solid rgba(255,255,255,.05);flex-shrink:0;background:rgba(0,0,0,.2);"></div>
          <div id="tl-label-list" style="flex:1;overflow:hidden;"></div>
        </div>
        <!-- Scrollable track area -->
        <div style="flex:1;overflow:auto;position:relative;" id="tl-scroll-container" onscroll="syncLabelScroll(this)">
          <!-- Ruler -->
          <div id="tl-ruler" style="height:20px;background:rgba(255,255,255,.03);border-bottom:.5px solid rgba(255,255,255,.06);position:sticky;top:0;z-index:10;"></div>
          <!-- Tracks -->
          <div id="tl-tracks" style="position:relative;min-width:max-content;min-height:80px;">
            <!-- Playhead -->
            <div id="tl-playhead" style="position:absolute;top:0;bottom:0;width:2px;background:#00d4ff;z-index:20;pointer-events:none;left:0;">
              <div style="position:absolute;top:-1px;left:-5px;width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-top:7px solid #00d4ff;"></div>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    // CONSTANTS
    // ══════════════════════════════════════════════════════════════════════════════
    const API = 'https://aabstudio-production.up.railway.app'; window.API = API;
    ...
    function nav(pg) {
      ...
      updateNavState();
      // Fire nav event so all hooks can respond without overriding nav
      try { document.dispatchEvent(new CustomEvent('aab_nav', { detail: { page: pg } })); } catch(e) {}
      if (pg === 'dashboard-pg') loadDashboard();
      if (pg === 'scenes-pg') renderSceneBoard();
      if (pg === 'edit-pg') renderEditSuite();
      if (pg === 'record-pg') {
      initStudio();
      setTimeout(function() { if (typeof updateTP === 'function') updateTP(); }, 200);
    }
    ...
    function updateNavState() {
      ...
      // Home page nav buttons - switch between logged-out and logged-in state
      show('home-signin-btn', !li); show('home-start-btn', !li);
      show('home-dashboard-btn', li); show('home-studio-btn', li);
      if (li) {
    ...
    function tpToggleAdaptive() { TP.adaptive = !TP.adaptive; var btn = document.getElementById('tp-adapt-btn'); if (btn) btn.textContent = TP.adaptive ? '🎤 Adaptive: on' : '🎤 Adaptive: off'; toast(TP.adaptive ? 'Adaptive sync ON — speak and it follows' : 'Adaptive sync OFF', 'info'); } function remoteSend(cmd, val) {
      // Send via Socket.IO to server, which relays to the studio
      if (typeof socket !== 'undefined' && socket.connected) {
        ...
        if (dot) { dot.style.background = '#00d4ff'; setTimeout(function(){ dot.style.background = '#10b981'; }, 200); }
      } else {
        // Fallback: localStorage polling (when socket not available)
        var sid = window._remoteSid;
        if (sid) localStorage.setItem('aab_remote_' + sid, cmd + ':' + (val||'') + ':' + Date.now());
        // Try to reconnect socket
        if (typeof socket !== 'undefined') socket.connect();
      }
    }// ══ MODALS ════════════════════════════════════════════════════════════════════
    ...
    function updateStudioClips() {
      // Update saved clips counter in SESSION panel
      var clipCount = Object.keys(PROJECT.clips || {}).length;
      var countEl = document.getElementById('st-clips-saved');
      if (countEl) countEl.textContent = clipCount;
      // Push to edit suite timeline immediately
    ...
    function updateTP() {
      // Only guard during active scrolling (not on page load)
      if (window._aabTpScrolling) return;
      var scene   = PROJECT.scenes[STUDIO.currentScene || 0];
      var prevSc  = PROJECT.scenes[(STUDIO.currentScene || 0) - 1];
      var nextSc  = PROJECT.scenes[(STUDIO.currentScene || 0) + 1];
      ...
      // No scenes - guide user
      if (!PROJECT.scenes || !PROJECT.scenes.length) {
        curEl.innerHTML = '<div style="color:rgba(255,255,255,.25);font-size:16px;text-align:center;padding:20px;">' +
          '📝 No script yet — go to Script to add your content, then come back to record.' +
          '</div>';
      ...
      // No narration in current scene
      var narration = (scene && scene.narration) ? scene.narration.trim() : '';
      if (!narration) {
        curEl.innerHTML = '<div style="color:rgba(255,255,255,.3);font-size:15px;text-align:center;padding:20px;">' +
          '⚠️ Scene ' + ((STUDIO.currentScene || 0) + 1) + ' has no script. Select this scene in Script editor to add text.' +
          '</div>';
        if (prevEl) prevEl.innerHTML = '';
        if (nextEl) nextEl.innerHTML = '';
        return;
      ...
      var words = narration.split(/\s+/).filter(Boolean);
      var spans = words.map(function(w, i) {
        return '<span id="aab_tw' + i + '" class="tp-word" style="display:inline;margin-right:0.3em;color:rgba(255,255,255,0.55);transition:color 0.08s,font-weight 0.08s;">' +
          w.replace(/&/g,'&amp;').replace(/</g,'&lt;') + '</span>';
      }).join('');

      curEl.innerHTML = '<div class="tp-cur-inner" id="tp-cur-inner">' + spans + '</div>';

      // Prev scene — last line
      if (prevEl) {
        var pt = prevSc ? (prevSc.narration || '').trim() : '';
        if (pt.length > 80) pt = '…' + pt.slice(-70);
        prevEl.innerHTML = pt ? '<span style="color:rgba(255,255,255,.2);font-size:13px;">' + pt + '</span>' : '';
      }
      // Next scene — first line
      if (nextEl) {
        var nt = nextSc ? (nextSc.narration || '').trim() : '';
        if (nt.length > 80) nt = nt.slice(0,70) + '…';
        nextEl.innerHTML = nt ? '<span style="color:rgba(255,255,255,.2);font-size:13px;">' + nt + '</span>' : '';
      }
    }
    ...
    function toggleTpPanel() {
      ...
      var collapseBtn = document.querySelector('[onclick*="toggleTpPanel"]');
      if (!tp) return;
      var collapsed = tp.classList.contains('tp-collapsed');
      if (collapsed) {
        // EXPAND
        tp.classList.remove('tp-collapsed');
        tp.style.minHeight = '';
        tp.style.height = '';
        var inner = tp.querySelector('#tp-cur, .tp-cur-inner, [id*="tp-cur"]');
        if (inner) inner.style.display = '';
        var controls = tp.querySelector('.tp-controls, .studio-tp-controls');
        if (controls) controls.style.display = '';
        if (icon) icon.textContent = '▲';
        if (collapseBtn) collapseBtn.title = 'Collapse teleprompter';
      } else {
        // COLLAPSE — keep only the handle visible
        tp.classList.add('tp-collapsed');
        tp.style.height = '28px';
        tp.style.minHeight = '28px';
        tp.style.overflow = 'hidden';
        if (icon) icon.textContent = '▼';
        if (collapseBtn) collapseBtn.title = 'Expand teleprompter';
      }
    }
    ...
    async function doExport() {
      ...
      if (EXPORT_METHOD === 'heygen' || EXPORT_METHOD === 'kling' || EXPORT_METHOD === 'piapi') {
        ...
        try {
          ...
          toast('Scene ' + (sc ? sc.num : '?') + ' generation failed — check logs', 'error');
        }
        return;
      }
    ...
    var APS = {
      ...
      settings: {
        ...
        shot: 'medium',
        motion: 'static',
        provider: 'did',  // D-ID default: uses uploaded photo, fastest
        ratio: '16:9',
        bgLocked: true,
        stability: 0.5,
        clarity: 0.75
    ...
    function aps_selectScene(idx) {
      ...
      if (durEl) var _wc = (sc.narration || '').trim().split(/\s+/).filter(Boolean).length;
      var _wpm = 130; // average speaking WPM
      var _estDur = Math.round(_wc / (_wpm / 60));
      durEl.textContent = sc.duration + 's · ' + _wc + ' words (~' + _estDur + 's to read)';
      ...
      try {
        ...
        var vidR = await fetch(API + '/api/presenter', {
          ...
          body: JSON.stringify({
            ...
            referenceImageMimeType: APS.photoMimeType || 'image/jpeg',
        ...
        var maxPolls = 180; var pollCount = 0; // 180 × 5s = 15 min max
        while (pollCount < maxPolls && !APS.stopRequested) {
          // Poll every 5s for faster feedback (HeyGen takes 2-8min per scene)
          await new Promise(function (r) { setTimeout(r, 5000); });
          pollCount++;
          // Update progress display every poll
          var pct = Math.min(90, Math.round((pollCount / maxPolls) * 100));
          aps_showStep(sc.num, 3, 3, 'Scene ' + sc.num + ' rendering… ' + (pollCount*5) + 's (' + pct + '%)');
          try {
            var statR = await fetch(API + '/api/presenter-status?taskId=' + encodeURIComponent(sc.taskId));
            var statD = await statR.json();
            console.log('Poll scene', sc.num, ':', statD.status || 'unknown', 'done:', statD.done, 'hasUrl:', !!statD.videoUrl);

            if (statD.done && statD.videoUrl) {
              sc.status = 'done';
              sc.videoUrl = statD.videoUrl;
              APS.results[sc.id] = statD.videoUrl;
              // Safely update UI — don't let errors here break the return
              try { aps_updateSceneRow(sc); } catch(e) {}
              try { aps_addClipToBar(sc); } catch(e) {}
              try { aps_addClipToTimeline(sc); } catch(e) {}
              try { if (sceneIdx === APS.currentIdx) aps_selectScene(sceneIdx); } catch(e) {}
              toast('Scene ' + sc.num + ' ready ✓', 'ok');
        ...
        try {
          var finalR = await fetch(API + '/api/presenter-status?taskId=' + encodeURIComponent(sc.taskId));
          var finalD = await finalR.json();
          if (finalD.done && finalD.videoUrl) {
            sc.status = 'done'; sc.videoUrl = finalD.videoUrl;
            APS.results[sc.id] = finalD.videoUrl;
            aps_updateSceneRow(sc); aps_addClipToBar(sc); aps_addClipToTimeline(sc);
            toast('Scene ' + sc.num + ' video ready ✓', 'ok');
            return true;
          }
        } catch(fe) {}
        throw new Error('Timed out after ' + (maxPolls * 5) + 's waiting for video');
    ...
    function aps_showStep(sceneNum, step, totalSteps, message) {
      ...
      var stepEl= document.getElementById('aps-progress-step');
      ...
      // Scene label — clean, no technical details
      if (label) label.textContent = 'Scene ' + sceneNum + ' of ' + APS.scenes.length;

      // Step description — human readable
      var stepNames = ['', 'Generating voice…', 'Compositing scene…', 'Creating video…'];
      if (stepEl) stepEl.textContent = message || stepNames[step] || 'Processing…';
      ...
      if (queue) {
        queue.textContent = done + ' done' + (failed ? ' · ' + failed + ' failed' : '') + (pending ? ' · ' + pending + ' pending' : '');
      }
    }
    ...
    function aps_addClipToTimeline(sc) {
      ...
      if (!PROJECT.clips) PROJECT.clips = {};
      var clipKey = sc.id || ('aps_' + sc.num + '_' + Date.now());
      PROJECT.clips[clipKey] = {
        sceneId: sc.id,
        sceneNum: sc.num,
        ...
        duration: sc.duration || 8,
        source: 'ai-presenter',
        provider: APS.settings ? APS.settings.provider : 'heygen',
        voice: APS.settings ? APS.settings.voiceName : '',
        bg: APS.settings ? APS.settings.bg : '',
        generatedAt: new Date().toISOString()
      };
      // Also add to APS_CLIPS for the Go to Edit Suite function
      if (window.APS_CLIPS) {
        APS_CLIPS[clipKey] = PROJECT.clips[clipKey];
        try {
          var saved = JSON.parse(localStorage.getItem('aab_aps_clips') || '{}');
          saved[clipKey] = APS_CLIPS[clipKey];
          localStorage.setItem('aab_aps_clips', JSON.stringify(saved));
        } catch(e) {}
      }
  ...
  <div class="pg" id="privacy-pg"
    ...
    <div style="color:var(--ink);font-size:14px;line-height:1.8;">
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">1. Information We Collect</h2>
      <p>We collect: (a) Account information — your email address and password hash; (b) Project data — titles, scene scripts, and settings; (c) Payment information — processed securely by Stripe, we never store card details; (d) Anonymous usage analytics to improve the service.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">2. Video Recordings</h2>
      <p>Videos recorded in the Recording Studio are processed entirely in your browser and stored locally on your device. We do not upload or store your webcam recordings. AI-generated videos are processed by third-party services (HeyGen, D-ID) under their own privacy policies.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">3. How We Use Your Data</h2>
      <p>Your data is used solely to: provide and maintain the Platform; process payments; improve features; send service communications. We do not sell your personal data to any third party.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">4. Data Storage and Security</h2>
      <p>Account data is stored securely via Supabase (AWS EU). We use HTTPS encryption in transit and encryption at rest. Access to user data is restricted to authorised personnel only.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">5. Third-Party Services</h2>
      <p>AABStudio integrates with: Supabase (auth/database), Stripe (payments), Anthropic Claude (AI assistance), ElevenLabs (voice), HeyGen and D-ID (AI video generation), Imgur (image hosting). Each operates under its own privacy policy.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">6. Camera and Microphone</h2>
      <p>Camera and microphone access is used solely for local recording in the Recording Studio. No camera or microphone data is transmitted to our servers.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">7. Your Rights (GDPR)</h2>
      <p>You have the right to access, correct, delete, or export your personal data. To exercise these rights, contact us at the address below. You may delete your account at any time from Account Settings.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">8. Cookies</h2>
      <p>We use essential cookies for authentication sessions only. We do not use tracking or advertising cookies.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">9. Data Retention</h2>
      <p>Your account data is retained while your account is active. Payment records are retained as required by law. You may request full deletion at any time.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">10. Contact</h2>
      <p>For privacy questions: <a href="mailto:privacy@aabstudio.ai" style="color:var(--accent);">privacy@aabstudio.ai</a></p>
    </div>
  </div>
  ...
  <div class="pg" id="terms-pg"
    ...
    <p style="color:var(--muted);font-size:13px;margin-bottom:24px;">Last updated: April 2026</p>
    <div style="color:var(--ink);font-size:14px;line-height:1.8;">
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">1. Acceptance of Terms</h2>
      <p>By accessing or using AABStudio.ai you agree to be bound by these Terms of Service. If you do not agree, please do not use the Platform.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">2. Service Description</h2>
      <p>AABStudio.ai is a cloud-based video production platform providing AI-assisted teleprompter recording, AI presenter generation (via ElevenLabs, HeyGen and D-ID), and video editing tools.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">3. Account Registration</h2>
      <p>You must create an account to access full features. You are responsible for maintaining the security of your credentials and for all activities under your account. You must provide accurate information during registration.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">4. Subscription and Billing</h2>
      <p>Paid plans are billed monthly or annually. A 7-day free trial is available — your payment method will be charged at the end of the trial unless cancelled before it ends. Cancellation takes effect at the end of the current billing period. Refunds are assessed on a case-by-case basis.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">5. Credits</h2>
      <p>AI generation features consume credits. Credits are non-refundable, non-transferable, and do not roll over between billing periods. Credit usage is displayed before each generation action.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">6. Permitted Use</h2>
      <p>You may use AABStudio solely for lawful purposes. You may not create illegal, defamatory or infringing content, attempt to reverse-engineer the Platform, or resell access without written permission.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">7. Intellectual Property</h2>
      <p>You retain full ownership of all content you create — scripts, recordings, and exported videos. AABStudio retains all rights in the Platform itself, its code, and underlying technology.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">8. AI-Generated Content</h2>
      <p>AI-generated videos are subject to the terms of the underlying services (HeyGen, D-ID, ElevenLabs). You are responsible for compliance with applicable laws regarding synthetic media disclosure.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">9. Limitation of Liability</h2>
      <p>The Platform is provided "as is". To the maximum extent permitted by law, AABStudio shall not be liable for indirect, incidental, or consequential damages arising from use of the Platform.</p>
      <h2 style="font-size:18px;font-weight:600;margin:24px 0 10px;">10. Governing Law</h2>
      ...
      <p style="margin-top:24px;">Contact: <a href="mailto:legal@aabstudio.ai" style="color:var(--accent);">legal@aabstudio.ai</a></p>
    </div>
  </div>
  ...
  <div id="ai-presenter-studio" class="pg-full" style="background:#0a0e1a;flex-direction:column;overflow:hidden;">
    ...
    <div
      ...
      <div style="display:flex;align-items:center;gap:10px;">
        ...
        <button onclick="aps_stopQueue()" id="aps-stop-btn"
          ...
          Stop</button>
        <!-- Go to Edit Suite — always visible, enabled after clips generated -->
        <button id="aps-go-edit-btn-top" onclick="aps_goToEditSuite()"
          style="background:rgba(16,185,129,.15);color:rgba(16,185,129,.5);border:1px solid rgba(16,185,129,.25);padding:7px 16px;border-radius:6px;cursor:not-allowed;font-size:12px;font-weight:700;transition:all .2s;"
          disabled title="Generate scenes first, then go to Edit Suite">
          🎬 Go to Edit Suite →
        </button>
    ...
    <div style="display:flex;flex:1;overflow:hidden;">
      ...
      <div
        ...
        <!-- Progress bar -->
        <!-- Generation Progress Panel with animation -->
        <div id="aps-progress-wrap" style="display:none;width:100%;max-width:640px;margin-top:12px;padding:14px;background:rgba(0,212,255,.04);border:.5px solid rgba(0,212,255,.2);border-radius:12px;">
          <style>@keyframes aps-spin{to{transform:rotate(360deg)}}@keyframes aps-pulse{0%,100%{opacity:.5}50%{opacity:1}}</style>
          <!-- Header: spinner + scene label + percent -->
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
            <div id="aps-spinner" style="width:20px;height:20px;border:2.5px solid rgba(0,212,255,.15);border-top-color:#00d4ff;border-radius:50%;animation:aps-spin .8s linear infinite;flex-shrink:0;"></div>
            <div style="flex:1;min-width:0;">
              <div id="aps-progress-label" style="font-size:12px;font-weight:600;color:rgba(255,255,255,.9);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"></div>
              <div id="aps-progress-step" style="font-size:10px;color:rgba(0,212,255,.7);margin-top:1px;animation:aps-pulse 2s ease infinite;"></div>
            </div>
            <span id="aps-progress-pct" style="font-size:13px;font-weight:700;color:#00d4ff;flex-shrink:0;"></span>
          </div>
          <!-- Progress bar -->
          <div style="height:5px;background:rgba(255,255,255,.06);border-radius:3px;overflow:hidden;margin-bottom:8px;">
            <div id="aps-progress-bar" style="height:100%;background:linear-gradient(90deg,rgba(0,212,255,.7),#00d4ff);border-radius:3px;width:0%;transition:width .5s ease;box-shadow:0 0 8px rgba(0,212,255,.4);"></div>
          </div>
          <!-- Footer: queue count + stop button -->
          <div style="display:flex;align-items:center;justify-content:space-between;">
            <div id="aps-queue-status" style="font-size:10px;color:rgba(255,255,255,.3);"></div>
            <button id="aps-stop-btn" onclick="APS.stopRequested=true;this.textContent='Stopping…';this.disabled=true;"
              style="display:none;font-size:9px;padding:3px 10px;background:rgba(255,60,60,.1);border:.5px solid rgba(255,60,60,.3);color:rgba(255,100,100,.8);border-radius:6px;cursor:pointer;">⏹ Stop</button>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        <div class="aps-section">
          ...
          <!-- STEP 1: Saved Characters (shown first so returning users just pick one) -->
          <div class="aps-label" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
            <span>Saved Characters</span>
            <button onclick="aps_renderCharGrid()" style="background:none;border:none;color:rgba(255,255,255,.3);font-size:9px;cursor:pointer;padding:0;">↻ Refresh</button>
          </div>
          <div id="aps-char-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px;max-height:180px;overflow-y:auto;">
            <div style="grid-column:1/-1;text-align:center;color:rgba(255,255,255,.2);font-size:10px;padding:12px 0;">
          ...
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
            <div style="flex:1;height:.5px;background:rgba(255,255,255,.08);"></div>
            <span style="font-size:9px;color:rgba(255,255,255,.2);white-space:nowrap;">OR ADD NEW</span>
            <div style="flex:1;height:.5px;background:rgba(255,255,255,.08);"></div>
          </div>

          <!-- STEP 2: Photo upload (for new character or to update existing) -->
          <div id="aps-photo-drop" style="position:relative;margin-bottom:8px;">
            <!-- Upload button -->
            <label id="aps-upload-label"
              style="display:flex;align-items:center;justify-content:center;gap:8px;padding:10px;border:1.5px dashed rgba(255,255,255,.15);border-radius:8px;cursor:pointer;font-size:12px;color:#9ca3af;transition:all .2s;background:rgba(255,255,255,.02);"
              onmouseover="this.style.borderColor='#00d4ff';this.style.color='#00d4ff';"
              onmouseout="this.style.borderColor='rgba(255,255,255,.15)';this.style.color='#9ca3af';">
              <span>📷</span>
              <span id="aps-upload-label-text">Upload presenter photo</span>
              <input id="aps-photo-input" type="file" accept="image/*" style="display:none;"
                onchange="aps_loadPhoto(this)">
            </label>
            <!-- Preview (shown after upload) -->
            <div id="aps-photo-preview-wrap" style="display:none;margin-top:8px;display:none;align-items:center;gap:10px;">
              <img id="aps-photo-preview" style="width:52px;height:52px;border-radius:50%;object-fit:cover;border:2px solid #00d4ff;flex-shrink:0;">
              <div style="flex:1;min-width:0;">
                <div id="aps-photo-name" style="font-size:10px;color:rgba(255,255,255,.6);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></div>
                <button onclick="document.getElementById('aps-photo-input').click()" style="font-size:9px;color:rgba(0,212,255,.6);background:none;border:none;cursor:pointer;padding:0;margin-top:2px;">↻ Change photo</button>
              </div>
            </div>
          </div>

          <!-- STEP 3: Character name + Save -->
          <div class="aps-label">Character Name</div>
          <div style="display:flex;gap:6px;margin-bottom:4px;">
            <input id="aps-char-name" type="text" class="aps-select" placeholder="e.g. John Anchor"
              style="flex:1;" oninput="APS.charName=this.value">
            <button onclick="aps_saveCharacter(document.getElementById('aps-char-name').value)"
              style="background:rgba(0,212,255,.15);border:.5px solid rgba(0,212,255,.3);color:var(--accent);border-radius:6px;padding:5px 10px;font-size:10px;cursor:pointer;white-space:nowrap;">💾 Save</button>
          </div>
          <div id="aps-char-name-display" style="font-size:9px;color:rgba(0,212,255,.5);margin-bottom:10px;">
            Name your presenter to save and reuse across projects
          </div>
        ...
        <div class="aps-section">
          ...
          <select id="aps-provider" class="aps-select" onchange="aps_onProviderChange(this.value)">
            <option value="did" selected>D-ID ⚡ (fast, uses your face)</option>
            <option value="heygen">HeyGen (stock avatar)</option>
            <option value="kling">Kling AI (motion video)</option>
          </select>
          <div id="aps-provider-note" style="font-size:9px;color:rgba(0,212,255,.6);margin-top:3px;">
            ⚡ D-ID animates your uploaded photo in 30-90s per scene
          </div>
    ...
    <div id="aps-clips-bar"
      style="height:90px;background:#0d1117;border-top:.5px solid #1f2937;display:flex;align-items:center;padding:0 12px;gap:10px;flex-shrink:0;">
      <div style="flex-shrink:0;display:flex;flex-direction:column;gap:4px;">
        <span style="font-size:9px;color:#4b5563;text-transform:uppercase;letter-spacing:.05em;">Generated Clips</span>
        <button id="aps-go-edit-btn" onclick="aps_goToEditSuite()"
          style="display:none;background:#10b981;color:#fff;border:none;border-radius:7px;padding:6px 14px;font-size:11px;font-weight:700;cursor:pointer;font-family:'Sora',sans-serif;">
          🎬 Go to Edit Suite →
        </button>
        <button onclick="nav('scenes-pg')"
          style="background:rgba(255,255,255,.06);color:rgba(255,255,255,.5);border:.5px solid rgba(255,255,255,.1);border-radius:6px;padding:3px 8px;font-size:10px;cursor:pointer;font-family:'Sora',sans-serif;">
          ← Scenes
        </button>
      </div>
      <div id="aps-clips-list" style="flex:1;display:flex;gap:8px;align-items:center;overflow-x:auto;min-height:70px;"></div>
...
<!-- ADDITIVE SCRIPTS -->
    ...
    <script id="aab-v5">
      ...
      // startRec/stopRec NOT patched here — originals handle state correctly
      // Patching stopRec causes duplicate saves and UI freeze (onstop fires multiple times)
      ...
      // stopRec: original at L7017 handles MediaRecorder.stop() + blob save + clip add
      // DO NOT override here — already handled
...
@media(max-width:768px){
  ...
  .studio-cam{width:100%!important;height:100%!important;object-fit:cover!important}
...
// Removed — nav hooks handled via aab_nav event
...
// stopRec override consolidated — side effects handled by original onstop handler
// DO NOT add another stopRec override here — it causes freeze from chained calls
// All clip saving, timeline refresh, and toasts happen inside the onstop handler above
...
function aab_showNextStep(targetPage, message) {
  // Show a brief toast only — no persistent overlay bar that blocks buttons
  if (message && typeof toast === 'function') toast(message, 'ok', 3000);
  // Keep a hidden element for any code that checks it
  var h = document.getElementById('aab-next-hint');
  if (!h) {
    h = document.createElement('div');
    h.id = 'aab-next-hint';
    h.style.display = 'none'; // always hidden
    document.body.appendChild(h);
  }
  h.textContent = message;
  h.onclick = function(){ if(typeof nav==='function') nav(targetPage); };
  clearTimeout(h._t);
...
<script>
      ...
      (function () {
        ...
        var _origStopRec = window.stopRec;
        // stopRec override removed — timeline rebuild happens via updateStudioClips
        // Adding more stopRec overrides causes chained calls and UI freeze
...
// nav hook removed — handled in original nav()
...
(function(){
  ...
  s.textContent=
    ...
    '.studio-ctrl-row{display:flex!important;align-items:stretch!important;overflow-x:auto!important;max-height:64px!important;}'+
    ...
    '.panel-group-body{display:flex!important;align-items:center!important;gap:4px!important;padding:3px 10px!important;flex:1!important;}'+
    '.studio-wrap{display:flex!important;flex-direction:column!important;height:100vh!important;}'+
    '.studio-main{flex:1!important;display:flex!important;overflow:hidden!important;min-height:0!important;max-height:calc(100vh - 46px - 128px)!important;}';
  document.head.appendChild(s);
})();
...
function _aabTpScrollStart() {
  ...
  window._aabTpActive = true;

  var slider  = document.getElementById('tp-spd-range');
  var wpm     = parseInt(slider ? slider.value : 120) || 120;
  var modeSel = document.getElementById('tp-mode-sel');
  var tpMode  = (modeSel ? modeSel.value : null) || window._tpMode || 'scroll';
  window._tpMode = tpMode;
  var msPerWord = 60000 / wpm;

  var scene = PROJECT && PROJECT.scenes && PROJECT.scenes[STUDIO.currentScene || 0];
  var text  = scene ? (scene.narration || '').trim() : '';
  if (!text) { toast('No script text in this scene', 'error'); return; }

  var tpCur = document.getElementById('tp-cur');
  if (!tpCur) { console.warn('tp-cur not found'); return; }

  // ── BUILD: Wrap every word in a span ──────────────────────────────────────
  var words = text.split(/\s+/).filter(Boolean);
  tpCur.innerHTML = words.map(function(w, i) {
    return '<span id="aab_tw' + i + '" style="' +
      'display:inline;' +
      'margin-right:0.32em;' +
      'transition:color 0.08s,text-shadow 0.08s,font-weight 0.08s;' +
      'color:rgba(255,255,255,0.35);' +
    '">' + w.replace(/&/g,'&amp;').replace(/</g,'&lt;') + '</span>';
  }).join('');

  // ── LAYOUT: fixed reading line in upper-third of TP container ─────────────
  // The text scrolls UP continuously. A fixed "reading cursor" line sits
  // at 30% from top. Words above it are dimmed (read), word at line = bright,
  // words below = slightly dimmed (coming up).
  var tpWrap = tpCur.parentElement;
  if (tpWrap) {
    tpWrap.style.overflow = 'hidden';
    tpWrap.style.position = 'relative';
  }
  // Add reading guide line (semi-transparent horizontal bar)
  var guide = document.getElementById('tp-reading-guide');
  if (!guide) {
    guide = document.createElement('div');
    guide.id = 'tp-reading-guide';
    guide.style.cssText = [
      'position:absolute',
      'left:0','right:0',
      'top:30%',
    ...
    if (tpWrap) tpWrap.appendChild(guide);
  }
  guide.style.display = 'block';

  // ── SCROLL: continuous smooth upward scroll ───────────────────────────────
  // Scroll speed = pixels per millisecond, calculated from WPM and average word width
  // We calculate how far to scroll per word, then drive it with requestAnimationFrame

  var startTime = null;
  var lastWi    = -1;
  var containerH = tpWrap ? tpWrap.offsetHeight : 300;
  var readingY    = containerH * 0.30; // reading guide at 30% from top

  // Give the layout time to render before measuring word positions
  requestAnimationFrame(function() {
    requestAnimationFrame(function() {
      // Measure total scroll distance = from first word to last word
      var firstSpan = document.getElementById('aab_tw0');
      var lastSpan  = document.getElementById('aab_tw' + (words.length - 1));
      if (!firstSpan || !lastSpan) return;

      // Total scroll = distance from first word top to last word top
      var totalScroll = lastSpan.offsetTop - firstSpan.offsetTop;
      // Total time for all words
      var totalTime = words.length * msPerWord;
      // Pixels per millisecond
      var pxPerMs = totalTime > 0 ? totalScroll / totalTime : 0;

      // Initial scroll: put first word at the reading line
      var initialOffset = firstSpan.offsetTop - readingY;
      tpCur.style.transform = 'translateY(' + (-initialOffset) + 'px)';
      tpCur.style.transition = 'none';
      tpCur.style.willChange = 'transform';
      ...
      function frame(ts) {
        ...
        var scrolled   = elapsed * pxPerMs;
        var currentOffset = initialOffset + scrolled;
        tpCur.style.transform = 'translateY(' + (-currentOffset) + 'px)';
        ...
        if (wi !== lastWi) {
          ...
          if (lastWi >= 0) {
            var prev = document.getElementById('aab_tw' + lastWi);
            if (prev) {
              prev.style.color      = 'rgba(255,255,255,0.25)';
          ...
          if (cur) {
            ...
            cur.style.fontWeight = '700';
            cur.style.textShadow = '0 0 20px rgba(255,255,255,0.9), 0 0 40px rgba(0,212,255,0.5)';
          }
          // Soften upcoming words (next 3)
          for (var ni = wi + 1; ni <= wi + 3 && ni < words.length; ni++) {
            var nxt = document.getElementById('aab_tw' + ni);
            if (nxt && nxt.style.color !== '#ffffff') {
              // Gradually brighter as they approach
              var alpha = 0.25 + (ni - wi - 1) * 0.1;
              nxt.style.color      = 'rgba(255,255,255,' + Math.min(0.55, alpha) + ')';
              nxt.style.fontWeight = '';
              nxt.style.textShadow = 'none';
            }
          }
          lastWi = wi;
        }
        ...
        if (wi >= words.length - 1 && elapsed >= words.length * msPerWord) {
          if (guide) guide.style.display = 'none';
          var nextScene = (STUDIO.currentScene || 0) + 1;
          if (nextScene < (PROJECT.scenes || []).length) {
            if (window._aab_syncing && typeof studioNext === 'function') studioNext();
            setTimeout(function() {
              if (window._aab_practice || window._aab_syncing) _aabTpScrollStart();
            }, 300);
          } else {
            window._aab_practice = false;
            window._aab_syncing  = false;
            if (STUDIO.recording && typeof stopRec === 'function') stopRec();
            toast('All scenes complete ✓', 'ok');
          }
          return;
        }
...
window.stopRec = function() {
  // Safety: release UI lock after 5s if MediaRecorder.onstop doesn't fire
  var _releaseTimer = setTimeout(function() {
    if (typeof STUDIO !== 'undefined' && STUDIO.recording) {
      STUDIO.recording = false;
      if (typeof setStudioState === 'function') setStudioState('READY');
      console.warn('stopRec: safety release fired (onstop did not fire in 5s)');
    }
  }, 5000);
  // Store timer so onstop can cancel it
  if (typeof STUDIO !== 'undefined') STUDIO._releaseTimer = _releaseTimer;
...
// Wire studio buttons via aab_nav event (no nav override needed)
document.addEventListener('aab_nav', function(e) {
  var pg = e.detail && e.detail.page;
  // edit-pg needs display:flex not block (it's a flex column layout)
  if (pg === 'edit-pg') {
    var ep = document.getElementById('edit-pg');
  ...
  if (pg === 'record-pg') {
    setTimeout(function(){
      ...
      if (typeof _addTpBgControl === 'function') _addTpBgControl();
    }, 300);
  }
  if (pg === 'edit-pg') {
    setTimeout(function(){
...
window.aps_generateScene = async function(idx) {
  ...
  try {
    ...
    var vr = await fetch((window.API||API) + '/api/voice', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        text:            sc.narration,
    ...
    var pr = await fetch((window.API||API) + '/api/presenter', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    var pollMs   = 10000;
    var maxPolls = 180; // 180 × 10s = 30 min
    
    for (var i = 0; i < maxPolls && !APS.stopRequested; i++) {
      await new Promise(function(r){ setTimeout(r, pollMs); });
      
      try {
        var sr = await fetch((window.API||API) + '/api/presenter-status?taskId=' + encodeURIComponent(pd.taskId));
        var stat = await sr.json();
        var elapsed = Math.round((i+1) * pollMs / 1000);
...
function startSceneStatusPolling() {
  ...
  _sceneStatusPollTimer = setInterval(async function() {
    ...
    try {
      ...
      var r = await fetch((window.API||API) + '/api/project/scenes-status?projectId=' + PROJECT.id, {
...
window.doPublish = async function() {
  ...
  try {
    var r = await fetch((window.API||API) + '/api/creatomate/sync', {
      method:  'POST',
    ...
    for (var i = 0; i < maxPolls; i++) {
      ...
      var sr   = await fetch((window.API||API) + '/api/creatomate/status/' + taskId);
      var stat = await sr.json();
...
// Scene DB sync hooks — attached to nav via event listener instead of override
// This avoids breaking the nav chain
document.addEventListener('aab_nav', function(e) {
  var pg = e.detail && e.detail.page;
  if (!pg) return;
  if (pg === 'dashboard-pg' || pg === 'home') {
    if (typeof stopSceneStatusPolling === 'function') stopSceneStatusPolling();
  }
  if (pg === 'edit-pg' || pg === 'export-pg') {
    if (typeof syncSceneDB             === 'function') syncSceneDB();
    if (typeof startSceneStatusPolling === 'function') startSceneStatusPolling();
  }
});
...
// ══════════════════════════════════════════════════════════════════
// SOCKET.IO — Real-time remote control across devices
// Replaces BroadcastChannel (same-device only) with proper WebSocket
// Phone → Railway server → Desktop, any network, any device
// ══════════════════════════════════════════════════════════════════

(function initSocketIO() {
  // Load Socket.IO client from CDN
  if (window._aabSocket) return; // Already initialised
  ...
  script.onerror = function() { console.warn('Socket.IO CDN load failed — remote will use localStorage fallback'); };
  document.head.appendChild(script);
})();

function connectSocket() {
  if (window._aabSocket) return;
  try {
    var socket = io(API, {
    ...
    window._aabSocket = socket;

    socket.on('connect', function() {
      console.log('Socket.IO connected:', socket.id);
      // Join project room immediately
      if (PROJECT && PROJECT.id) {
        socket.emit('join_project', { project_id: PROJECT.id });
        socket.emit('subscribe_project', { project_id: PROJECT.id });
      }
      // Update remote connectivity dot
      var dot = document.getElementById('remote-conn-dot');
      if (dot) { dot.style.background = '#10b981'; dot.title = 'Socket.IO connected'; }
      var tpDot = document.getElementById('tp-conn-dot');
      if (tpDot) { tpDot.style.background = '#10b981'; tpDot.textContent = '● Connected'; }
    });
    ...
    // ── RECEIVE remote control commands on desktop ─────────────────
    // Phone sends → server → desktop receives here
    socket.on('command_to_teleprompter', function(data) {
      var action = data.action || data.cmd;
      console.log('Remote command received:', action);

      // Update remote page connectivity dot
      var remoteDot = document.getElementById('remote-conn-dot');
      if (remoteDot) remoteDot.style.background = '#10b981';

      switch (action) {
        case 'play':
          if (window._aab_practice || window._aab_syncing) {
            // Already running — ignore
          } else if (typeof startPracticeMode === 'function') {
            startPracticeMode();
          }
          break;
        case 'pause':
          if (typeof _aabTpStop === 'function') _aabTpStop();
          break;
        case 'slower':
          var spd = document.getElementById('tp-spd-range');
          if (spd) { spd.value = Math.max(30, parseInt(spd.value) - 10); spd.dispatchEvent(new Event('input')); }
          break;
        case 'faster':
          var spd2 = document.getElementById('tp-spd-range');
          if (spd2) { spd2.value = Math.min(300, parseInt(spd2.value) + 10); spd2.dispatchEvent(new Event('input')); }
          break;
        case 'next_scene':
          if (typeof studioNext === 'function') studioNext();
          break;
        case 'prev_scene':
          if (typeof studioPrev === 'function') studioPrev();
          break;
        case 'restart':
          if (typeof _aabTpStop === 'function') _aabTpStop();
          setTimeout(function() { if (typeof startPracticeMode==='function') startPracticeMode(); }, 200);
          break;
        case 'stop':
          if (typeof _aabTpStop === 'function') _aabTpStop();
          if (typeof stopRec        === 'function') stopRec();
          break;
        case 'set_speed':
          var spd3 = document.getElementById('tp-spd-range');
          if (spd3 && data.value) { spd3.value = data.value; spd3.dispatchEvent(new Event('input')); }
          break;
      }
    });

    // ── RECEIVE clip_ready — HeyGen video done, add to timeline ───
    socket.on('clip_ready', function(data) {
      console.log('Clip ready via WebSocket:', data.videoId);
      if (!data.videoUrl) return;

      // Check if this taskId is one we're waiting for
      var waiting = false;
      if (APS && APS.scenes) {
        APS.scenes.forEach(function(sc) {
          if (sc.taskId === data.taskId || sc.taskId === 'heygen-' + data.videoId) {
            sc.status   = 'done';
            sc.videoUrl = data.videoUrl;
            waiting     = true;
            if (typeof aps_updateSceneRow === 'function') aps_updateSceneRow(sc);
          }
        });
      ...
      if (waiting || !PROJECT.clips['ai_' + data.videoId]) {
        // Add to project clips
        if (!PROJECT.clips) PROJECT.clips = {};
        PROJECT.clips['ai_' + data.videoId] = {
          url:       data.videoUrl,
          videoUrl:  data.videoUrl,
          sceneNum:  1,
          source:    'ai-presenter',
          sceneName: 'AI Presenter',
          sceneType: 'BATCH',
          duration:  30
        };
        if (typeof edit_buildPlaylist        === 'function') edit_buildPlaylist();
        ...
        if (typeof tl_buildTracksFromProject === 'function') tl_buildTracksFromProject();
        if (typeof saveProject               === 'function') saveProject();
        toast('✅ AI video ready — added to timeline', 'ok');
      }
    });

    // ── TP state sync — keep phone speed slider in sync ───────────
    socket.on('tp_state_update', function(data) {
      if (data.wpm) {
        var spd = document.getElementById('remote-spd');
        if (spd) { spd.value = data.wpm; }
        var spdLbl = document.getElementById('remote-spd-val');
        if (spdLbl) spdLbl.textContent = data.wpm + 'wpm';
      }
    });
...
// ── Override remoteSend to use Socket.IO (falls back to BroadcastChannel) ──
window.remoteSend = function(action, value) {
  var projectId = PROJECT && PROJECT.id;
  
  // Try Socket.IO first (cross-device)
  if (window._aabSocket && window._aabSocket.connected && projectId) {
    var payload = { project_id: projectId, action: action };
    if (value !== undefined) payload.value = value;
    window._aabSocket.emit('remote_control', payload);
    console.log('remoteSend via Socket.IO:', action);
  ...
  // Fallback: BroadcastChannel (same device only)
  try {
    var ch = new BroadcastChannel('aab_remote');
    ch.postMessage({ action: action, value: value, ts: Date.now() });
    ch.close();
  } catch(e) {}
  try { localStorage.setItem('aab_remote_cmd', JSON.stringify({ action, value, ts: Date.now() })); } catch(e) {}
  console.log('remoteSend via BroadcastChannel (fallback):', action);
};

// ── Join project room when project changes ────────────────────────
var _aabSocketProjectId = null;
setInterval(function() {
  if (!window._aabSocket || !window._aabSocket.connected) return;
  var pid = PROJECT && PROJECT.id;
  if (pid && pid !== _aabSocketProjectId) {
    _aabSocketProjectId = pid;
    window._aabSocket.emit('join_project',      { project_id: pid });
    window._aabSocket.emit('subscribe_project', { project_id: pid });
    console.log('Socket.IO: joined project room', pid);
  }
}, 3000);
...
// ═══════════════════════════════════════════════════════════════════
// PERSISTENCE LAYER v2.0 — Projects & Scenes DB
// Source of truth: Supabase projects + scenes tables
// Auto-saves on every nav, auto-loads on sign-in
// ═══════════════════════════════════════════════════════════════════

// ── DB SAVE — project + all scenes ───────────────────────────────
window.db_saveProject = async function() {
  if (!APP.user) return;
  var token = APP.user.accessToken || APP.user.token || (APP.user.session && APP.user.session.access_token);
  if (!token) return;
  ...
  try {
    ...
    var projectPayload = {
      ...
      user_id:               APP.user.id,
      title:                 PROJECT.title || 'My Video',
      reference_image_id:    window.APS && APS.talkingPhotoId || null,
      global_background_url: window.APS && APS.bgUrl || null,
      background_type:       window.APS && APS.settings && APS.settings.bg || 'news-studio',
      voice_id:              window.APS && APS.settings && APS.settings.voiceId || 'EXAVITQu4vr4xnSDxMaL',
      voice_name:            window.APS && APS.settings && APS.settings.voiceName || null,
      voice_gender:          window.APS && APS.settings && APS.settings.voiceGender || 'male',
      stability:             window.APS && APS.settings && APS.settings.stability || 0.85,
      clarity:               window.APS && APS.settings && APS.settings.clarity || 0.80,
      aspect_ratio:          window.APS && APS.settings && APS.settings.ratio || '16:9',
      framing_style:         window.APS && APS.settings && APS.settings.framing || 'news-desk',
      provider:              window.APS && APS.settings && APS.settings.provider || 'heygen',
      wpm:                   PROJECT.wpm || 150,
      scene_duration:        PROJECT.sceneDuration || 8,
      status:                PROJECT.status || 'draft',
    };

    var supa = getSupa();
    var { error: projErr } = await supa.from('projects').upsert(projectPayload, { onConflict: 'id' });
    if (projErr) throw new Error('Project save: ' + projErr.message);

    // Upsert scenes
    if (PROJECT.scenes && PROJECT.scenes.length > 0) {
      var sceneRows = PROJECT.scenes.map(function(sc, idx) {
        return {
          id:               sc.id,
          project_id:       PROJECT.id,
          scene_order:      sc.sceneNumber || idx + 1,
          script_text:      sc.narration || '',
          scene_type:       sc.type || 'MAIN',
          duration:         sc.duration || 8,
          notes:            sc.notes || null,
          status:           sc.status || 'pending',
          user_recording_url: (PROJECT.clips && PROJECT.clips[sc.id] && (PROJECT.clips[sc.id].url || PROJECT.clips[sc.id].videoUrl)) || sc.userRecordingUrl || null,
          ai_video_url:     sc.videoUrl || sc.aiVideoUrl || null,
          camera_motion:    sc.cameraMotion || 'static',
        };
      });
      var { error: sceneErr } = await supa.from('scenes').upsert(sceneRows, { onConflict: 'id' });
      if (sceneErr) console.warn('Scenes save:', sceneErr.message);
    }

    console.log('DB: project + scenes saved');
  } catch(e) {
    console.warn('db_saveProject failed (non-critical):', e.message);
  }
};

// ── DB LOAD — fetch project + scenes on sign-in ───────────────────
window.db_loadLatestProject = async function() {
  if (!APP.user) return null;
  try {
    var supa = getSupa();
    // Get most recent project
    var { data: projects, error: pErr } = await supa
      .from('projects')
      .select('*')
    ...
    if (pErr || !projects || !projects.length) return null;
    var proj = projects[0];

    // Get its scenes
    var { data: scenes, error: sErr } = await supa
      .from('scenes')
      .select('*')
      .eq('project_id', proj.id)
      .order('scene_order', { ascending: true });

    // Reconstruct PROJECT from DB
    PROJECT.id             = proj.id;
    PROJECT.title          = proj.title || 'My Video';
    PROJECT.wpm            = proj.wpm || 150;
    PROJECT.sceneDuration  = proj.scene_duration || 8;
    PROJECT.status         = proj.status || 'draft';

    // Reconstruct APS settings
    if (window.APS) {
      APS.talkingPhotoId = proj.reference_image_id || null;
      APS.bgUrl          = proj.global_background_url || null;
      if (!APS.settings) APS.settings = {};
      APS.settings.bg       = proj.background_type || 'news-studio';
      APS.settings.voiceId  = proj.voice_id || 'EXAVITQu4vr4xnSDxMaL';
      APS.settings.voiceName  = proj.voice_name || null;
      APS.settings.voiceGender = proj.voice_gender || 'male';
      APS.settings.stability  = proj.stability || 0.85;
      APS.settings.clarity    = proj.clarity || 0.80;
      APS.settings.ratio      = proj.aspect_ratio || '16:9';
      APS.settings.framing    = proj.framing_style || 'news-desk';
      APS.settings.provider   = proj.provider || 'heygen';
      // Restore photo ID to localStorage
      if (APS.talkingPhotoId) {
        try { localStorage.setItem('aps_talking_photo_id', APS.talkingPhotoId); } catch(e) {}
      }
    }

    // Reconstruct scenes
    if (scenes && scenes.length) {
      PROJECT.scenes = scenes.map(function(sc) {
        return {
          id:          sc.id,
          sceneNumber: sc.scene_order,
          type:        sc.scene_type || 'MAIN',
          narration:   sc.script_text || '',
          duration:    sc.duration || 8,
          notes:       sc.notes || '',
          status:      sc.status || 'pending',
          videoUrl:    sc.ai_video_url || null,
          userRecordingUrl: sc.user_recording_url || null,
          assets:      [],
        };
      });

      // Rebuild clips from scenes with recordings
      if (!PROJECT.clips) PROJECT.clips = {};
      scenes.forEach(function(sc, idx) {
        if (sc.user_recording_url) {
          PROJECT.clips[sc.id] = {
            url: sc.user_recording_url, videoUrl: sc.user_recording_url,
            sceneId: sc.id, sceneNum: sc.scene_order,
            sceneName: sc.script_text ? sc.script_text.slice(0,40) : 'Scene '+sc.scene_order,
            sceneType: sc.scene_type || 'MAIN',
            source: 'recording', duration: sc.duration || 8
          };
        }
        if (sc.ai_video_url) {
          PROJECT.clips['ai_'+sc.id] = {
            url: sc.ai_video_url, videoUrl: sc.ai_video_url,
            sceneId: sc.id, sceneNum: sc.scene_order,
            sceneName: 'AI — ' + (sc.script_text||'').slice(0,30),
            sceneType: sc.scene_type || 'MAIN',
            source: 'ai-presenter', duration: sc.duration || 8
          };
        }
      });
    }

    console.log('DB: loaded project', proj.id, '—', (scenes||[]).length, 'scenes');
    return proj;
  } catch(e) {
    console.warn('db_loadLatestProject failed:', e.message);
    return null;
  }
};

// ── DB SAVE SCENES STATUS after recording/generation ─────────────
window.db_updateSceneStatus = async function(sceneId, status, videoUrl) {
  if (!APP.user || !sceneId) return;
  try {
    var supa = getSupa();
    var update = { status: status };
    if (videoUrl) {
      if (status === 'recorded') update.user_recording_url = videoUrl;
      if (status === 'generated') update.ai_video_url = videoUrl;
    }
    await supa.from('scenes').update(update).eq('id', sceneId);
  } catch(e) {
    console.warn('db_updateSceneStatus:', e.message);
  }
};
...
document.addEventListener('aab_nav', function(e) {
  ...
  clearTimeout(window._dbSaveTimer);
  window._dbSaveTimer = setTimeout(function() {
    window.db_saveProject();
  }, 1000);
});

// ── PATCH setUser to load project from DB on sign-in ─────────────
var _origSetUser = window.setUser;
window.setUser = function(u) {
  if (typeof _origSetUser === 'function') _origSetUser(u);
  // After user is set, load their most recent project from DB
  setTimeout(async function() {
    if (!APP.user) return;
    var proj = await window.db_loadLatestProject();
    if (proj) {
      // Refresh UI with loaded project data
      if (typeof renderProjectGrid    === 'function') renderProjectGrid();
      if (typeof renderSceneBoard     === 'function') renderSceneBoard();
      if (typeof edit_buildPlaylist   === 'function') edit_buildPlaylist();
      if (typeof edit_renderClipStrip === 'function') edit_renderClipStrip();
      toast('Project restored: ' + (PROJECT.title||'My Video'), 'ok');
    }
  }, 500);
};

// ── PATCH stopRec/studioNext to auto-save scene status ───────────
// After a recording, update the scene status in Supabase immediately
var _origUpdateStudioClips = window.updateStudioClips;

// ── REMOTE CONTROL — Socket.IO pair_devices ───────────────────────
// Override Socket.IO join to use pair_devices + command events
// matching the spec exactly
document.addEventListener('aab_nav', function(e) {
  var pg = e.detail && e.detail.page;
  if (pg === 'remote-pg' && window._aabSocket && PROJECT.id) {
    // Phone joins the project room
    window._aabSocket.emit('pair_devices', {
      project_id: PROJECT.id,
      device: 'phone'
    });
    // Show green dot
  ...
  if ((pg === 'record-pg' || pg === 'teleprompter-pg') && window._aabSocket && PROJECT.id) {
    // Desktop joins the project room
    window._aabSocket.emit('pair_devices', {
      project_id: PROJECT.id,
      device: 'desktop'
    });
...
// Handle command events from phone (matching spec's remote_execute)
if (window._aabSocket) {
  window._aabSocket.on('remote_execute', function(data) {
    var action = data.action || data.cmd;
    console.log('remote_execute received:', action);
    switch(action) {
      case 'play':    if (typeof startPracticeMode==='function') startPracticeMode(); break;
      case 'pause':   if (typeof _aabTpStop==='function') _aabTpStop(); break;
      case 'next_scene': if (typeof studioNext==='function') studioNext(); break;
      case 'prev_scene': if (typeof studioPrev==='function') studioPrev(); break;
      case 'slower':
        var sp=document.getElementById('tp-spd-range');
        if(sp){sp.value=Math.max(30,parseInt(sp.value)-10);sp.dispatchEvent(new Event('input'));}
        break;
      case 'faster':
        var sp2=document.getElementById('tp-spd-range');
        if(sp2){sp2.value=Math.min(300,parseInt(sp2.value)+10);sp2.dispatchEvent(new Event('input'));}
        break;
      case 'restart':
        if(typeof _aabTpStop==='function') _aabTpStop();
        setTimeout(function(){if(typeof startPracticeMode==='function')startPracticeMode();},200);
...
// ── MOBILE REMOTE MODE ────────────────────────────────────────────
// Switch to large-button layout on narrow screens
(function initMobileRemote() {
  if (window.innerWidth >= 768) return;
  // On mobile, the remote page gets a special class
  var style = document.createElement('style');
  style.textContent = [
    '.remote-btn { min-height: 80px !important; font-size: 22px !important; }',
    '#remote-pg { padding-bottom: env(safe-area-inset-bottom, 20px) !important; }',
    '.remote-grid { grid-template-columns: 1fr 1fr !important; gap: 16px !important; }',
    '.remote-speed-row { padding: 20px !important; }',
    '/* Hide timeline on mobile remote */',
    '@media (max-width: 767px) {',
    '  #edit-pg .tl-tools-group, #edit-pg #tl-scroll-container { font-size: 10px; }',
    '  #edit-pg #edit-preview-pane { width: 100% !important; }',
    '  #edit-pg #timeline-panel { display: none; }',
    '}',
  ].join('\n');
  document.head.appendChild(style);

  // Use remoteSend which now goes via Socket.IO
  window._mobileRemote = true;
  document.addEventListener('aab_nav', function(e) {
    if (e.detail && e.detail.page === 'remote-pg') {
      document.body.classList.add('mobile-remote-mode');
    } else {
      document.body.classList.remove('mobile-remote-mode');
    }
  });
})();

// ── Wire remoteSend to use Socket.IO 'command' event (matches spec) ─
window.remoteSend = function(action, value) {
  var projectId = PROJECT && PROJECT.id;
  var payload = { project_id: projectId, action: action };
  if (value !== undefined) payload.value = value;
  ...
  // Try Socket.IO first — cross-device
  if (window._aabSocket && window._aabSocket.connected && projectId) {
    window._aabSocket.emit('command', payload);  // matches spec event name
    var dot = document.getElementById('remote-conn-dot');
    if (dot) dot.style.background = '#10b981';
    return;
  }
  ...
  try { localStorage.setItem('aab_remote_cmd', JSON.stringify({...payload, ts: Date.now()})); } catch(e) {}
};
...
// 1. Character naming & persistence
// 2. APS clips in chronological order to timeline
// 3. Timeline tools fully wired (trim, split, text, upload)
// 4. Phone remote fully wired
// 5. Toast cleanup — only on nav, not repeated button clicks
// 6. Text overlay tool in edit suite
// 7. Upload wiring (video, audio, image)
// 8. Timeline scrollbar fix
// ═══════════════════════════════════════════════════════════════════

// ── 1. CHARACTER NAMING & PERSISTENCE ─────────────────────────────
// Users can name their presenter and reuse across projects


window.aps_listCharacters = function() {
  return JSON.parse(localStorage.getItem('aps_characters') || '{}');
};

// Render character selector in APS sidebar

// APS page opened — restore character and render grid
document.addEventListener('aab_nav', function(e) {
  if (e && e.detail && e.detail.page === 'ai-presenter-studio') {
    setTimeout(function() {
      // Always render the character grid
      if (typeof aps_renderCharGrid === 'function') aps_renderCharGrid();

      // Restore last used character with full photo
      try {
        var chars = JSON.parse(localStorage.getItem('aab_characters') || '{}');
        var lastName = localStorage.getItem('aab_last_character');
        var charToLoad = (lastName && chars[lastName]) ? chars[lastName] : Object.values(chars)[0];

        if (charToLoad) {
          // Restore photo to APS state
          if (charToLoad.photoB64 && !APS.photoB64) {
            APS.photoB64 = charToLoad.photoB64;
            APS.photoMimeType = charToLoad.photoMimeType || 'image/jpeg';
            var dataUrl = 'data:' + (charToLoad.photoMimeType || 'image/jpeg') + ';base64,' + charToLoad.photoB64;

            // Large canvas
            var wrap = document.getElementById('aps-presenter-img-wrap');
            if (wrap) wrap.innerHTML = '<img src="' + dataUrl + '" style="width:100%;height:100%;object-fit:cover;object-position:top;border-radius:100px 100px 0 0;">';

            // Small panel preview
            var prev = document.getElementById('aps-photo-preview');
            var prevWrap = document.getElementById('aps-photo-preview-wrap');
            var labelTxt = document.getElementById('aps-upload-label-text');
            var nameElPrev = document.getElementById('aps-photo-name');
            if (prev) prev.src = dataUrl;
            if (prevWrap) prevWrap.style.display = 'flex';
            if (labelTxt) labelTxt.textContent = 'Photo loaded ✓';
            if (nameElPrev) nameElPrev.textContent = charToLoad.name + ' photo';
          ...
          // Restore settings
          if (charToLoad.talkingPhotoId) APS.talkingPhotoId = charToLoad.talkingPhotoId;
          if (APS.settings) {
            if (charToLoad.voiceId)   APS.settings.voiceId   = charToLoad.voiceId;
            if (charToLoad.voiceName) APS.settings.voiceName = charToLoad.voiceName;
            if (charToLoad.provider)  APS.settings.provider  = charToLoad.provider;
          }
          if (charToLoad.framing) APS.framing = charToLoad.framing;

          // Update name field
          APS.charName = charToLoad.name;
          var nameInput = document.getElementById('aps-char-name');
          if (nameInput) nameInput.value = charToLoad.name;

          // Update provider selector
          var provSel = document.getElementById('aps-provider');
          if (provSel && charToLoad.provider) {
            provSel.value = charToLoad.provider;
            if (typeof aps_onProviderChange === 'function') aps_onProviderChange(charToLoad.provider);
          }

          // Update status display
          var display = document.getElementById('aps-char-name-display');
          if (display) {
            display.textContent = '✓ Using: ' + charToLoad.name;
            display.style.color = 'rgba(0,212,255,.6)';
          }
        }
      } catch(ex) {
        console.warn('APS character restore failed:', ex.message);
      }
    }, 300);
  }
});

// ── 2. APS CLIPS → TIMELINE in chronological order ───────────────
// Override aps_addClipToBar to also push to edit suite in scene order
var _origAddClipToBar = window.aps_addClipToBar || function(){};

// ── 3. TIMELINE TOOLS — fully wired ──────────────────────────────

// Trim tool — drag clip edges to trim
window.tl_trim = function(clipId, edge, newTime) {
  if (!window.TL || !TL.tracks) return;
  TL.tracks.forEach(function(track) {
    var clip = track.clips && track.clips.find(function(c){ return c.id===clipId; });
    if (!clip) return;
    if (edge === 'in') {
      var delta = newTime - clip.start;
      clip.start    = newTime;
      clip.trimIn   = (clip.trimIn||0) + delta;
      clip.duration = Math.max(0.5, clip.duration - delta);
    } else {
      clip.trimOut  = clip.start + clip.duration - newTime;
      clip.duration = Math.max(0.5, newTime - clip.start);
    }
  });
  if (typeof tl_render === 'function') tl_render();
  toast('Trimmed ✓', 'ok');
...
// Split at playhead — works on selected clip
window.tl_split = function() {
  if (!window.TL || !TL.tracks) return;
  var ph = TL.playhead || 0;
  var splitDone = false;
  ...
  TL.tracks.forEach(function(track) {
    ...
    track.clips.forEach(function(clip) {
      ...
      if (ph > clip.start + 0.1 && ph < end - 0.1) {
        // Split this clip
        var left  = Object.assign({}, clip, { duration: ph - clip.start, id: clip.id + '_a' });
        var right = Object.assign({}, clip, { start: ph, duration: end - ph, id: clip.id + '_b', trimIn: (clip.trimIn||0) + (ph - clip.start) });
  ...
  if (splitDone) {
    if (typeof tl_render === 'function') tl_render();
    toast('Split at ' + (TL.playhead||0).toFixed(1) + 's ✓', 'ok');
  } else {
    toast('Move playhead inside a clip to split', 'info');
  }
};

// ── 3b. TEXT OVERLAY TOOL ─────────────────────────────────────────
window.tl_addText = function() {
  if (!window.TL) return;
  var text = prompt('Enter text overlay:', 'Your text here');
  if (!text) return;
  
  var overlayTrack = TL.tracks && TL.tracks.find(function(t){ return t.type==='overlay' || t.id==='overlays'; });
  if (!overlayTrack) {
    overlayTrack = { id:'overlays_'+Date.now(), type:'overlay', name:'🔤 Text', muted:false, volume:1, clips:[] };
    if (TL.tracks) TL.tracks.push(overlayTrack);
  }
  ...
  overlayTrack.clips = overlayTrack.clips || [];
  overlayTrack.clips.push(newClip);
  TL.selectedClip = newClip.id;
  if (typeof tl_render === 'function') tl_render();
  
  // Show text properties panel
  tl_showTextProps(newClip);
  toast('Text overlay added ✓', 'ok');
};

window.tl_showTextProps = function(clip) {
  var panel = document.getElementById('tl-text-props');
  if (!panel) {
    panel = document.createElement('div');
    panel.id = 'tl-text-props';
    panel.style.cssText = 'position:fixed;right:16px;bottom:120px;background:#1a2235;border:.5px solid rgba(255,255,255,.12);border-radius:10px;padding:14px;width:220px;z-index:1000;';
    panel.innerHTML = '<div style="font-size:11px;font-weight:700;color:rgba(255,255,255,.6);margin-bottom:10px;">TEXT PROPERTIES</div>' +
      '<label style="font-size:10px;color:rgba(255,255,255,.4);">Text</label><br>' +
      '<input id="tp-text-val" type="text" style="width:100%;margin-bottom:8px;background:rgba(255,255,255,.08);border:.5px solid rgba(255,255,255,.12);color:#fff;border-radius:4px;padding:4px 6px;font-size:11px;">' +
      '<label style="font-size:10px;color:rgba(255,255,255,.4);">Size</label><br>' +
      '<input id="tp-text-size" type="range" min="12" max="80" value="24" style="width:100%;margin-bottom:8px;accent-color:var(--accent)">' +
      '<label style="font-size:10px;color:rgba(255,255,255,.4);">Color</label><br>' +
      '<input id="tp-text-color" type="color" value="#ffffff" style="width:100%;height:30px;margin-bottom:8px;">' +
      '<label style="font-size:10px;color:rgba(255,255,255,.4);">Position</label><br>' +
      '<select id="tp-text-pos" style="width:100%;margin-bottom:10px;background:rgba(255,255,255,.08);border:.5px solid rgba(255,255,255,.12);color:#fff;border-radius:4px;padding:4px 6px;font-size:11px;">' +
        '<option value="bottom">Bottom</option><option value="top">Top</option><option value="center">Center</option>' +
      '</select>' +
      '<div style="display:flex;gap:8px;">' +
        '<button onclick="tl_applyTextProps()" style="flex:1;background:var(--accent);color:#000;border:none;border-radius:6px;padding:6px;font-size:11px;cursor:pointer;">Apply</button>' +
        '<button onclick="window._closeTLTextProps()" style="flex:1;background:rgba(255,255,255,.1);color:#fff;border:none;border-radius:6px;padding:6px;font-size:11px;cursor:pointer;">Close</button>' +
      '</div>';
    document.body.appendChild(panel);
  }
  
  var textInput = document.getElementById('tp-text-val');
  if (textInput && clip) textInput.value = clip.text || '';
  panel.style.display = 'block';
  panel._clipId = clip.id;
};

window._closeTLTextProps = function(){ var x=document.getElementById('tl-text-props'); if(x)x.style.display='none'; };
window.tl_applyTextProps = function() {
  var panel = document.getElementById('tl-text-props');
  if (!panel || !panel._clipId) return;
  var clipId = panel._clipId;
  
  TL.tracks && TL.tracks.forEach(function(track) {
    var clip = track.clips && track.clips.find(function(c){ return c.id===clipId; });
    if (!clip) return;
    clip.text     = document.getElementById('tp-text-val').value;
    clip.fontSize = parseInt(document.getElementById('tp-text-size').value);
    clip.color    = document.getElementById('tp-text-color').value;
    clip.position = document.getElementById('tp-text-pos').value;
  });
...
// ── 3c. UPLOAD WIRING — video, audio, image ───────────────────────

// Wire the bottom import buttons in edit suite
document.addEventListener('aab_nav', function(e) {
  if (e.detail && e.detail.page !== 'edit-pg') return;
  setTimeout(function() {
    var videoBtn = document.querySelector('input[accept="video/*"][onchange*="importToTimeline"]');
    var audioBtn = document.querySelector('input[accept="audio/*"][onchange*="importToTimeline"]');
    // Already wired via HTML — just ensure function exists
  }, 200);
});

// ── 4. PHONE REMOTE — fully wired ─────────────────────────────────
// Override remoteSend to use Socket.IO AND wire the remote page buttons
window.remoteSend = function(action, value) {
  var projectId = PROJECT && PROJECT.id;
  var payload = { project_id: projectId, action: action };
  if (value !== undefined) payload.value = value;

  // Socket.IO — cross-device real-time
  if (window._aabSocket && window._aabSocket.connected && projectId) {
    window._aabSocket.emit('command', payload);
    var dot = document.getElementById('remote-conn-dot');
    if (dot) { dot.style.background = '#10b981'; dot.title = 'Connected ✓'; }
    return;
  }
  // BroadcastChannel fallback
  try { var ch=new BroadcastChannel('aab_remote'); ch.postMessage(payload); ch.close(); } catch(e) {}
  try { localStorage.setItem('aab_remote_cmd', JSON.stringify(Object.assign({},payload,{ts:Date.now()}))); } catch(e) {}
};

// Wire remote page buttons to execute locally (same device) AND via Socket.IO
function wireRemoteButtons() {
  var remPage = document.getElementById('remote-pg');
  if (!remPage) return;
  
  // Play/pause
  var playBtn = remPage.querySelector('[onclick*="play"]') || remPage.querySelector('[onclick*="pause"]');
  
  // Wire BroadcastChannel receiver for same-device remote
  try {
    var remCh = new BroadcastChannel('aab_remote');
    remCh.onmessage = function(e) {
      var data = e.data || {};
      executeRemoteCommand(data.action, data.value);
      var dot = document.getElementById('remote-conn-dot');
      if (dot) dot.style.background = '#10b981';
    };
  } catch(e) {}
  
  // Also poll localStorage for cross-page remote
  if (!window._remotePoller) {
    window._remotePoller = setInterval(function() {
      var cmd = null;
      try { cmd = JSON.parse(localStorage.getItem('aab_remote_cmd')||'null'); } catch(e) {}
      if (cmd && cmd.ts && (Date.now() - cmd.ts) < 3000) {
        try { localStorage.removeItem('aab_remote_cmd'); } catch(e) {}
        executeRemoteCommand(cmd.action, cmd.value);
      }
    }, 500);
  }
}
...
function executeRemoteCommand(action, value) {
  switch(action) {
    case 'play':
      if (typeof startPracticeMode === 'function') startPracticeMode();
      break;
    case 'pause':
      if (typeof _aabTpStop === 'function') _aabTpStop();
      break;
    ...
    case 'restart':
      ...
      break;
    case 'set_speed':
      var sp3 = document.getElementById('tp-spd-range');
      if (sp3 && value) { sp3.value = value; sp3.dispatchEvent(new Event('input')); }
      break;
  }
}
window.executeRemoteCommand = executeRemoteCommand;
...
document.addEventListener('aab_nav', function(e) {
  if (e.detail && e.detail.page === 'remote-pg') {
    wireRemoteButtons();
    // Join Socket.IO room
    if (window._aabSocket && PROJECT.id) {
      window._aabSocket.emit('pair_devices', { project_id: PROJECT.id, device: 'phone' });
    }
    var dot = document.getElementById('remote-conn-dot');
    if (dot) { dot.style.background = '#f59e0b'; dot.title = 'Waiting...'; }
  }
});

// Socket.IO remote_execute handler
if (window._aabSocket) {
  window._aabSocket.off('remote_execute');
  window._aabSocket.on('remote_execute', function(data) {
    executeRemoteCommand(data.action, data.value);
  });
  window._aabSocket.off('command_to_teleprompter');
  window._aabSocket.on('command_to_teleprompter', function(data) {
    executeRemoteCommand(data.action || data.cmd, data.value);
  });
}

// ── 5. TOAST CLEANUP — remove toasts that fire on every button click ─
// Toasts should fire on COMPLETION, not on every button press
// Wrap toast so duplicate toasts within 2s are suppressed
var _lastToastMsg = '', _lastToastTs = 0;
var _origToast = window.toast;
window.toast = function(msg, type, dur) {
  var now = Date.now();
  // Suppress duplicate toasts within 2 seconds
  if (msg === _lastToastMsg && (now - _lastToastTs) < 2000) return;
  _lastToastMsg = msg;
  _lastToastTs  = now;
  if (typeof _origToast === 'function') _origToast(msg, type, dur);
};

// ── 6. TIMELINE SCROLL FIX ────────────────────────────────────────
// Add CSS to prevent timeline from cutting off
(function fixTimelineLayout() {
  var style = document.createElement('style');
  style.textContent = [
    '/* Timeline full height and no overflow cutoff */',
    '#edit-pg { height: 100vh; overflow: hidden; display: flex; flex-direction: column; }',
    '#tl-scroll-container { overflow: auto; flex: 1; min-height: 0; }',
    '#tl-tracks { min-height: 300px; }',
    '/* Clip strip scrollable */',
    '#edit-clips-strip { overflow-x: auto; scrollbar-width: thin; scrollbar-color: rgba(255,255,255,.15) transparent; }',
    '/* Timeline clip draggable */',
    '.tl-clip { cursor: pointer; transition: opacity .1s; }',
    '.tl-clip:hover { opacity: .85; }',
    '.tl-clip.selected { outline: 2px solid var(--accent); }',
    '/* Remote buttons large for mobile */',
    '.remote-btn { min-height: 64px !important; font-size: 20px !important; font-weight: 700 !important; }',
    '/* APS clips strip */',
    '#aps-clips-list { display: flex; gap: 10px; overflow-x: auto; padding: 8px; min-height: 80px; }',
  ].join('\n');
  document.head.appendChild(style);
})();

// ── 7. APS CLIP ORDERING — chronological in strip ─────────────────
// Override edit_renderClipStrip to always sort by sceneNum
var _origRenderStrip = window.edit_renderClipStrip;

// ── 8. HeyGen — improve reference image accuracy ─────────────────
// The server needs to describe the reference image for HeyGen
// This is a server-side fix but we can improve the prompt we send
window.aps_getImageDescription = async function(b64) {
  if (!b64) return '';
  try {
    // Use Claude to describe the presenter image for better HeyGen matching
    var r = await fetch(API + '/api/describe-image', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
// Fix: db_loadLatestProject and scene polling should only fire when signed in
// Wrap db_saveProject to guard against missing user
var _origDbSave = window.db_saveProject;
window.db_saveProject = async function() {
  if (!APP || !APP.user || !PROJECT || !PROJECT.id) return; // Never fire without auth
  if (typeof _origDbSave === 'function') return _origDbSave();
};

// Fix: /api/project/list called before auth — guard it
var _origLoadProj = window.db_loadLatestProject;
window.db_loadLatestProject = async function() {
  if (!APP || !APP.user) return null;
  var token = APP.user.accessToken || APP.user.token || 
              (APP.user.session && APP.user.session.access_token);
  if (!token) return null;
  if (typeof _origLoadProj === 'function') return _origLoadProj();
  return null;
};

// Fix: aps_loadPhoto — on photo upload failure, continue with reference image mode
// Don't block the workflow if HeyGen talking_photo creation fails
var _origLoadPhoto = window.aps_loadPhoto;

// Fix: aps_generateAll — restore photo from localStorage before generating
var _origGenAll = window.aps_generateAll;

// Fix: scene status polling — only start when signed in and have project
var _origStartPolling = window.startSceneStatusPolling;
window.startSceneStatusPolling = function() {
  if (!APP || !APP.user || !PROJECT || !PROJECT.id) return;
  if (typeof _origStartPolling === 'function') _origStartPolling();
};



// ═══════════════════════════════════════════════════════════════════
// PERSISTENT STORAGE — Characters + Assets saved to Supabase
// Works across devices, sessions, and browsers
// ═══════════════════════════════════════════════════════════════════

function getAuthToken() {
  if (!APP || !APP.user) return null;
  return APP.user.accessToken || APP.user.token ||
         (APP.user.session && APP.user.session.access_token) || null;
}

// ── CHARACTER SAVE/LOAD — Supabase persistent ─────────────────────
window.aps_saveCharacter = async function(name) {
  if (!name || !name.trim()) { toast('Enter a character name first', 'error'); return; }
  name = name.trim();

  // Build character data object
  var charData = {
    name:             name,
    photoB64:         APS.photoB64 ? APS.photoB64.slice(0, 80000) : (function(){
      // Try to get from localStorage if APS.photoB64 is temporarily null
      var saved = localStorage.getItem('aps_presenter_photo');
      return saved ? saved.split(',')[1] || saved : null;
    }()),
    photoMimeType:    APS.photoMimeType || 'image/jpeg',
    talkingPhotoId:   APS.talkingPhotoId || null,
    voiceId:          APS.settings && APS.settings.voiceId   || null,
    voiceName:        APS.settings && APS.settings.voiceName || null,
    stability:        APS.settings && APS.settings.stability || 0.85,
    clarity:          APS.settings && APS.settings.clarity   || 0.80,
    provider:         APS.settings && APS.settings.provider  || 'did',
    framing:          APS.framing  || 'medium',
    bgType:           APS.bgType   || 'news-studio',
    savedAt:          Date.now()
  };
  ...
  try {
    var chars = JSON.parse(localStorage.getItem('aab_characters') || '{}');
    chars[name] = charData;
    localStorage.setItem('aab_characters', JSON.stringify(chars));
    // Also save photo separately for quick restore
    if (charData.photoB64) {
      localStorage.setItem('aab_photo_' + name, charData.photoB64);
    }
    // Update selector
    if (typeof aps_renderCharSelector === 'function') aps_renderCharSelector();
    toast('✓ Character "' + name + '" saved', 'ok');
    localStorage.setItem('aab_last_character', name);
    localStorage.setItem('aps_current_char', name);
  } catch(lsErr) {
    console.warn('Character localStorage save failed:', lsErr);
    toast('Character save failed (storage full?)', 'error');
    return;
  }

  // 2. Sync to server in background (non-blocking)
  try {
    var token = APP && APP.token;
    if (token) {
      fetch(API + '/api/character/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify(charData)
      }).then(function(r) {
        if (r.ok) console.log('Character synced to server:', name);
        else console.warn('Character server sync failed:', r.status);
      }).catch(function(e) { console.warn('Character server sync error:', e.message); });
    }
  } catch(e) { /* non-fatal */ }
};;

window.aps_loadCharacters = async function() {
  var all = {};
  ...
  if (token) {
    try {
      var r = await fetch((window.API||API) + '/api/characters', {
        headers: { 'Authorization': 'Bearer ' + token }
      });
      var d = await r.json();
      (d.characters || []).forEach(function(c) { all[c.name] = c; });
    } catch(e) { console.warn('Load characters from server failed:', e.message); }
  }

  // Merge with localStorage (covers offline / before server has table)
  try {
    var local = JSON.parse(localStorage.getItem('aps_characters') || '{}');
    Object.keys(local).forEach(function(name) {
      if (!all[name]) all[name] = local[name];
    });
  } catch(e) {}
...
window.aps_loadCharacter = async function(name) {
  var chars = await aps_loadCharacters();
  var char  = chars[name];
  if (!char) { toast('Character "' + name + '" not found', 'error'); return; }

  APS.charName        = char.name;
  APS.talkingPhotoId  = char.talking_photo_id || null;

  if (!APS.settings) APS.settings = {};
  if (char.voice_id)        APS.settings.voiceId     = char.voice_id;
  if (char.voice_name)      APS.settings.voiceName   = char.voice_name;
  if (char.voice_gender)    APS.settings.voiceGender = char.voice_gender;
  if (char.stability)       APS.settings.stability   = char.stability;
  if (char.clarity)         APS.settings.clarity     = char.clarity;
  if (char.framing)         APS.settings.framing     = char.framing;
  if (char.background_type) APS.settings.bg          = char.background_type;

  // Restore photo preview
  if (char.photo_url) {
    var preview = document.getElementById('aps-photo-preview');
    if (preview) { preview.src = char.photo_url; preview.style.display = 'block'; }
    var wrap = document.getElementById('aps-presenter-img-wrap');
    if (wrap) wrap.innerHTML = '<img src="'+char.photo_url+'" style="width:100%;height:100%;object-fit:cover;border-radius:100px 100px 0 0;">';
    APS.photoPreviewUrl = char.photo_url;
  }

  if (char.talking_photo_id) {
    try { localStorage.setItem('aps_talking_photo_id', char.talking_photo_id); } catch(e) {}
  }

  // Update name display and input
  var display = document.getElementById('aps-char-name-display');
  if (display) display.textContent = 'Loaded: ' + char.name;
  var nameInput = document.getElementById('aps-char-name');
  if (nameInput) nameInput.value = char.name;

  // Apply voice settings to dropdowns
  var voiceSel = document.getElementById('aps-voice-sel');
  if (voiceSel && char.voice_id) voiceSel.value = char.voice_id;

  localStorage.setItem('aps_current_char', char.name);
  toast('Character "' + char.name + '" loaded ✓', 'ok');
};

window.aps_renderCharSelector = async function() {
  var sel = document.getElementById('aps-char-select');
  if (!sel) return;
  var chars = await aps_loadCharacters();
  sel.innerHTML = '<option value="">— Select saved character —</option>';
  Object.keys(chars).sort().forEach(function(name) {
    var c   = chars[name];
    var opt = document.createElement('option');
    opt.value       = name;
    opt.textContent = name + (c.voice_gender ? ' (' + c.voice_gender + ')' : '');
    sel.appendChild(opt);
  });
  var currentChar = localStorage.getItem('aps_current_char');
  if (currentChar && chars[currentChar]) sel.value = currentChar;
};

// Load characters when APS opens
document.addEventListener('aab_nav', function(e) {
  if (e.detail && e.detail.page === 'ai-presenter-studio') {
    setTimeout(function() {
      aps_renderCharSelector();
      var last = localStorage.getItem('aps_current_char');
      if (last) {
        var d = document.getElementById('aps-char-name-display');
        if (d) d.textContent = 'Last used: ' + last;
        var ni = document.getElementById('aps-char-name');
        if (ni) ni.value = last;
      }
    }, 300);
  }
...
// ── ASSET LIBRARY — persistent uploaded files ─────────────────────
window.ASSET_LIBRARY = window.ASSET_LIBRARY || { items: [], loaded: false };


window.asset_loadLibrary = async function() {
  if (ASSET_LIBRARY.loaded) return;

  // Load from localStorage cache first (instant)
  try {
    var cached = JSON.parse(localStorage.getItem('aab_assets') || '[]');
    ASSET_LIBRARY.items = cached;
  } catch(e) {}
  ...
  if (token) {
    try {
      var r = await fetch((window.API||API) + '/api/assets', {
        headers: { 'Authorization': 'Bearer ' + token }
      });
      var d = await r.json();
      if (d.assets && d.assets.length) {
        // Merge server assets into library
        var serverIds = d.assets.map(function(a) { return a.id; });
        // Remove local items that are now on server, add server items
        ASSET_LIBRARY.items = d.assets.concat(
          ASSET_LIBRARY.items.filter(function(a) { return !serverIds.includes(a.id); })
        );
      }
    } catch(e) { console.warn('Load assets from server:', e.message); }
  }

  ASSET_LIBRARY.loaded = true;
  if (typeof asset_renderLibrary === 'function') asset_renderLibrary();
};

// Render asset library panel (used in scene board + edit suite)
window.asset_renderLibrary = function() {
  var panel = document.getElementById('asset-library-panel');
  if (!panel) return;
  var items = ASSET_LIBRARY.items;
  if (!items.length) {
    panel.innerHTML = '<div style="font-size:11px;color:rgba(255,255,255,.2);padding:16px;text-align:center;">No assets yet — upload images, audio or video</div>';
    return;
  }

  var html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:8px;">';
  items.slice(0, 40).forEach(function(a) {
    var icon = a.type === 'image' ? '🖼' : a.type === 'audio' ? '🎵' : a.type === 'video' ? '🎬' : '📁';
    var thumb = a.type === 'image' ? '<img src="'+a.url+'" style="width:100%;height:50px;object-fit:cover;border-radius:4px;display:block;">' :
                '<div style="width:100%;height:50px;background:rgba(255,255,255,.05);border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:24px;">'+icon+'</div>';
    html += '<div data-asset-id="'+a.id+'" onclick="asset_useInProject(this.dataset.assetId)" style="cursor:pointer;border-radius:6px;overflow:hidden;background:rgba(255,255,255,.04);border:.5px solid rgba(255,255,255,.08);" title="'+a.name+'">';
          + thumb
          + '<div style="font-size:9px;color:rgba(255,255,255,.5);padding:3px 5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+a.name+'</div>'
          + '</div>';
  });
  html += '</div>';
...
window.asset_useInProject = function(assetId) {
  var asset = ASSET_LIBRARY.items.find(function(a){ return a.id === assetId; });
  if (!asset) return;
  var url = asset.dataUrl || asset.url;

  if (asset.type === 'image') {
    // Add as background or overlay
    if (typeof setBg === 'function') setBg('custom', url);
    toast(asset.name + ' set as background', 'ok');
  } else if (asset.type === 'audio') {
    // Add to music track
    if (typeof loadBgMusic === 'function') loadBgMusic(null, url);
    toast(asset.name + ' added to audio track', 'ok');
  } else if (asset.type === 'video') {
    // Add to b-roll track
    if (window.TL) {
      var broll = TL.tracks && TL.tracks.find(function(t){ return t.id==='broll'; });
      if (broll) {
        broll.clips = broll.clips || [];
        broll.clips.push({ id:'asset_'+Date.now(), url, label:'🎬 '+asset.name, start: TL.playhead||0, duration:8, type:'video' });
        if (typeof tl_render === 'function') tl_render();
      }
    }
    toast(asset.name + ' added to B-Roll track', 'ok');
  }
};
...
document.addEventListener('aab_nav', function(e) {
  var pg = e.detail && e.detail.page;
  if (pg === 'dashboard-pg' || pg === 'scenes-pg' || pg === 'edit-pg') {
    if (APP && APP.user) asset_loadLibrary();
  }
});
...
// ═══════════════════════════════════════════════════════════════════
// CRITICAL FIXES — Session May 2
// 1. AI Presenter clips → Edit Suite (separate path, never mixed)
// 2. My Assets tab (images, audio, video, SFX) — persistent
// 3. Uploads actually work
// 4. Timeline horizontal scroll
// 5. Generated videos never lost on re-generation
// 6. Back to Scenes from both studios
// ═══════════════════════════════════════════════════════════════════

// ── 1. AI PRESENTER → EDIT SUITE (SEPARATE PATH) ─────────────────
// APS generates videos → stored as APS_CLIPS (separate from recording clips)
// When user clicks "Go to Edit Suite" from APS → loads ONLY APS clips
// When user goes to Edit Suite from Recording Studio → loads ONLY recording clips
// The two paths NEVER mix

window.APS_CLIPS = window.APS_CLIPS || {}; // AI-generated clips only
window.REC_CLIPS = window.REC_CLIPS || {}; // Recording studio clips only

// Override aps_addClipToBar to store in APS_CLIPS and show "Go to Edit Suite"
var _baseAddClipToBar = window.aps_addClipToBar || function(){};

// Navigate from APS to Edit Suite with ONLY AI clips (not recording clips)

// When going to Edit Suite from Recording Studio — load ONLY recording clips
document.addEventListener('aab_nav', function(e) {
  if (e.detail && e.detail.page === 'edit-pg') {
    // If coming from recording (not from APS), restore recording clips
    if (PROJECT._editMode !== 'ai') {
      // Merge in any recording clips
      Object.entries(PROJECT.clips||{}).forEach(function([k,v]) {
        if (v.source === 'recording') window.REC_CLIPS[k] = v;
      });
    }
    // Always ensure timeline is built
    setTimeout(function() {
      if (typeof tl_buildTracksFromProject === 'function') tl_buildTracksFromProject();
      if (typeof edit_renderClipStrip      === 'function') edit_renderClipStrip();
    }, 200);
  }
  if (e.detail && e.detail.page === 'record-pg') {
    PROJECT._editMode = 'recording';
  }
  if (e.detail && e.detail.page === 'ai-presenter-studio') {
    PROJECT._editMode = 'ai';
    // Restore any saved APS clips
    try {
      var saved = JSON.parse(localStorage.getItem('aab_aps_clips') || '{}');
      Object.assign(APS_CLIPS, saved);
    } catch(e) {}
  }
});

// Load saved APS clips on startup
(function() {
  try {
    var saved = JSON.parse(localStorage.getItem('aab_aps_clips') || '{}');
    Object.assign(APS_CLIPS, saved);
    console.log('APS: restored', Object.keys(saved).length, 'saved clips from localStorage');
  } catch(e) {}
})();

// ── 2. BATCH COMPLETION → ADD ALL CLIPS TO EDIT SUITE ────────────
// After batch generates, push ALL scenes to APS_CLIPS and show Go button
// aps_generateAll — direct implementation, no chaining
window.aps_generateAll = async function() {
  if (!APS || !APS.scenes || !APS.scenes.length) {
    toast('No scenes to generate', 'info'); return;
  }
  // Restore saved photo/voice if APS lost state
  if (!APS.photoB64) {
    var savedP = localStorage.getItem('aps_presenter_photo');
    if (savedP) APS.photoB64 = savedP.split(',')[1] || savedP;
  }
  if (!APS.talkingPhotoId) {
  ...
  if (stopBtn) { stopBtn.style.display = 'block'; stopBtn.textContent = '⏹ Stop'; stopBtn.disabled = false; }
  // Show progress panel immediately
  var wrap = document.getElementById('aps-progress-wrap');
  if (wrap) wrap.style.display = 'block';
  toast('Generating ' + APS.scenes.length + ' scenes…', 'info');
  var succeeded = 0, failed = 0;
  // Process in batches of 3 for faster generation
  var batchSize = 3;
  for (var i = 0; i < APS.scenes.length; i += batchSize) {
    if (APS.stopRequested) break;
    var batch = [];
    for (var b = i; b < Math.min(i + batchSize, APS.scenes.length); b++) {
      if (APS.scenes[b] && APS.scenes[b].status !== 'done') {
        batch.push(b);
      }
    }
    if (!batch.length) continue;
    toast('Generating scenes ' + (batch.map(function(x){return x+1;}).join(', ')) + '…', 'info');
    // Run batch in parallel
    var results = await Promise.allSettled(batch.map(function(idx) {
      return aps_generateScene(idx);
    }));
    results.forEach(function(r) {
      if (r.status === 'fulfilled' && r.value !== false) succeeded++;
      else failed++;
    });
    // Small gap between batches to avoid rate limiting
    if (i + batchSize < APS.scenes.length && !APS.stopRequested) {
  ...
  // Hide spinner in progress panel when done
  var spinner = document.getElementById('aps-spinner');
  if (spinner) spinner.style.display = 'none';
  toast('Done: ' + succeeded + ' generated' + (failed ? ', ' + failed + ' failed' : ''), succeeded > 0 ? 'ok' : 'error');
  // Show Go to Edit Suite button - make it visible
  var goBtn = document.getElementById('aps-go-edit-btn');
  if (goBtn && succeeded > 0) {
    goBtn.style.display = 'block';
    goBtn.textContent = '🎬 Go to Edit Suite (' + succeeded + ' clips) →';
  }
  // Enable topbar button
  var topBtn2 = document.getElementById('aps-go-edit-btn-top');
  if (topBtn2 && succeeded > 0) {
    topBtn2.disabled = false;
    topBtn2.style.background = '#10b981';
    topBtn2.style.color = '#fff';
    topBtn2.style.borderColor = '#10b981';
    topBtn2.style.cursor = 'pointer';
    topBtn2.style.boxShadow = '0 0 16px rgba(16,185,129,.5)';
    topBtn2.textContent = '🎬 Go to Edit Suite (' + succeeded + ') →';
  }
  var emptyMsg = document.getElementById('aps-clips-empty');
  if (emptyMsg && succeeded > 0) emptyMsg.style.display = 'none';
};

// When batch video completes, split into per-scene clips and add all to APS_CLIPS
window.aps_onBatchComplete = function(videoUrl, scenes) {
  if (!videoUrl || !scenes || !scenes.length) return;
  
  // The batch result is ONE video containing all scenes sequenced
  // Add it as a single clip (HeyGen doesn't give separate scene files)
  var totalDuration = scenes.reduce(function(t,s){ return t+(s.duration||8); }, 0);
  var key = 'aps_batch_' + Date.now();
  
  APS_CLIPS[key] = {
    url: videoUrl, videoUrl: videoUrl,
    sceneId: key, sceneNum: 1,
    sceneName: 'AI Presenter — ' + scenes.length + ' scenes',
    sceneType: 'BATCH', source: 'ai-presenter',
    duration: totalDuration, savedAt: Date.now(),
    sceneCount: scenes.length,
  };
  
  try {
    var saved = JSON.parse(localStorage.getItem('aab_aps_clips') || '{}');
    saved[key] = APS_CLIPS[key];
    localStorage.setItem('aab_aps_clips', JSON.stringify(saved));
  } catch(e) {}
  
  // Show in APS strip
  var fakeScene = { num: 1, type: 'BATCH', videoUrl, duration: totalDuration, id: key };
  if (typeof _baseAddClipToBar === 'function') _baseAddClipToBar(fakeScene);
  else window.aps_addClipToBar(fakeScene);
  
  toast('✅ ' + scenes.length + ' scenes complete! Click "Go to Edit Suite" to review.', 'ok');
};

// ── 3. MY ASSETS TAB — render in scenes page sidebar ──────────────
window.renderMyAssets = function(container) {
  if (!container) container = document.getElementById('asset-library-panel');
  if (!container) return;
  
  // Load from localStorage cache
  var items = [];
  try { items = JSON.parse(localStorage.getItem('aab_assets') || '[]'); } catch(e) {}
  
  // Also include saved APS clips as assets
  Object.values(APS_CLIPS).forEach(function(c) {
    if (c.url && !items.find(function(i){ return i.url===c.url; })) {
      items.unshift({ id: c.sceneId||'aps_'+Date.now(), name: c.sceneName, type:'video', url:c.url, savedAt:c.savedAt||Date.now() });
    }
  });
  
  if (!items.length) {
    container.innerHTML = '<div style="padding:20px;text-align:center;color:rgba(255,255,255,.3);font-size:12px;">No assets yet.<br>Upload images, audio, or video.</div>';
    return;
  }
  ...
  items.slice(0,60).forEach(function(a) {
    var icon = a.type==='image'?'🖼':a.type==='audio'?'🎵':a.type==='video'?'🎬':'📁';
    var thumb = a.type==='image'&&a.url&&!a.url.startsWith('local:')
      ? '<img src="'+a.url+'" style="width:100%;height:48px;object-fit:cover;display:block;">'
      : '<div style="height:48px;background:rgba(255,255,255,.05);display:flex;align-items:center;justify-content:center;font-size:20px;">'+icon+'</div>';
    var name = (a.name||'').slice(0,18);
    html += '<div data-asset-id="'+a.id+'" onclick="asset_useInProject(this.dataset.assetId)" '
      + 'style="cursor:pointer;border-radius:6px;overflow:hidden;background:rgba(255,255,255,.04);border:.5px solid rgba(255,255,255,.08);" '
      + 'title="'+a.name+'">'
      + thumb
      + '<div style="font-size:9px;color:rgba(255,255,255,.5);padding:2px 4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+name+'</div>'
      + '</div>';
  });
  html += '</div>';
...
// ── 4. UPLOAD ACTUALLY WORKING — wire all upload inputs ──────────

// ── 5. TIMELINE HORIZONTAL SCROLL FIX ─────────────────────────────
(function fixTimeline() {
  var style = document.createElement('style');
  style.id = 'aab-timeline-fix';
  style.textContent = [
    '#tl-scroll-container { overflow-x: auto !important; overflow-y: auto !important; min-height: 200px; }',
    '#tl-tracks { min-width: max-content; position: relative; }',
    '#tl-labels { overflow: hidden; min-height: 200px; }',
    '.tl-track-row { display: flex; align-items: stretch; min-height: 52px; border-bottom: .5px solid rgba(255,255,255,.04); }',
    '.tl-track-label { width: 88px; flex-shrink: 0; font-size: 10px; color: rgba(255,255,255,.5); padding: 0 8px; display: flex; align-items: center; border-right: .5px solid rgba(255,255,255,.07); background: rgba(0,0,0,.2); }',
    '.tl-track-rail { flex: 1; position: relative; min-height: 52px; min-width: 600px; }',
    '.tl-clip { position: absolute; height: 44px; top: 4px; border-radius: 6px; display: flex; align-items: center; padding: 0 8px; font-size: 10px; font-weight: 600; color: #fff; overflow: hidden; cursor: pointer; white-space: nowrap; transition: opacity .1s; }',
    '.tl-clip:hover { opacity: .85; }',
    '.tl-clip.selected { outline: 2px solid var(--accent); }',
    '#edit-pg { display: flex; flex-direction: column; height: 100vh; overflow: hidden; }',
    '#edit-clips-strip { height: 110px; flex-shrink: 0; overflow-x: auto; scrollbar-width: thin; }',
    /* Bottom */
    '.tl-bottom-bar { flex-shrink: 0; height: 36px; display: flex; align-items: center; gap: 8px; padding: 0 10px; border-top: .5px solid rgba(255,255,255,.05); background: rgba(0,0,0,.2); }',
    /* APS clips bottom strip */
    '#aps-clips-list { display: flex; gap: 10px; overflow-x: auto; padding: 8px; min-height: 84px; scrollbar-width: thin; }',
  ].join(' ');
  var existing = document.getElementById('aab-timeline-fix');
  if (existing) existing.remove();
  document.head.appendChild(style);
})();

// ── 6. BACK TO SCENES BUTTON — both studios ────────────────────────
// Add "← Scenes" button to studio bar and APS topbar if missing
(function addBackToScenesButtons() {
  // Studio bar
  document.addEventListener('aab_nav', function(e) {
    if (e.detail && e.detail.page === 'record-pg') {
      setTimeout(function() {
        var bar = document.querySelector('.studio-bar');
        if (bar && !bar.querySelector('#btn-back-scenes')) {
          var btn = document.createElement('button');
          btn.id = 'btn-back-scenes';
          btn.className = 'btn btn-ghost btn-sm';
          btn.textContent = '← Scenes';
          btn.onclick = function() { nav('scenes-pg'); };
    ...
    if (e.detail && e.detail.page === 'ai-presenter-studio') {
      setTimeout(function() {
        var topbar = document.querySelector('#ai-presenter-studio .aps-topbar, #ai-presenter-studio > div:first-child');
        if (topbar && !topbar.querySelector('#aps-back-scenes')) {
          var btn = document.createElement('button');
          btn.id = 'aps-back-scenes';
          btn.style.cssText = 'background:rgba(255,255,255,.08);border:.5px solid rgba(255,255,255,.15);color:rgba(255,255,255,.7);border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer;margin-right:8px;';
          btn.textContent = '← Scenes';
          btn.onclick = function() { nav('scenes-pg'); };
...
// ── 7. VERIFY HEYGEN ENTERPRISE REQUIREMENT ────────────────────────
// HeyGen talking_photo (custom face) requires Enterprise plan ($99+/mo)
// Creator plan ($24/mo) only gets stock avatars
// The server correctly tries to create a talking_photo_id but fails on Creator plan
// Users need to know this upfront
window.aps_checkHeyGenPlan = async function() {
  var token = typeof getAuthToken==='function' ? getAuthToken() : null;
  if (!token) return;
  try {
    var r = await fetch((window.API||API)+'/api/heygen/avatars', {
      headers: { 'Authorization': 'Bearer '+token }
    });
    var d = await r.json();
    var hasCustom = d.avatars && d.avatars.some(function(a){ return a.type==='talking_photo'||a.type==='photo'; });
    if (!hasCustom) {
      console.log('HeyGen: Creator plan detected — custom face requires Enterprise plan');
    }
    return hasCustom;
...
// ═══════════════════════════════════════════════════════════════════
// FINAL FIXES — Session May 2 (complete)
// ═══════════════════════════════════════════════════════════════════

// ── 1. sidebarTab — handle My Assets tab ──────────────────────────
var _origSidebarTab = window.sidebarTab;
window.sidebarTab = function(tab, btn) {
  // Handle my-assets specially
  var myPanel = document.getElementById('my-assets-panel');
  if (myPanel) myPanel.style.display = tab === 'my-assets' ? '' : 'none';
  ...
  if (typeof _origSidebarTab === 'function') _origSidebarTab(tab, btn);
};

function renderMyAssetsGrid() {
  var grid = document.getElementById('my-assets-grid');
  if (!grid) return;
  ...
  // Add saved APS clips
  Object.values(window.APS_CLIPS || {}).forEach(function(c) {
    if (c.url && !items.find(function(i){ return i.url===c.url; })) {
      items.unshift({ id:c.sceneId, name:c.sceneName||'AI Clip', type:'video', url:c.url, savedAt:c.savedAt||Date.now() });
    }
  });
  
  if (!items.length) {
    grid.innerHTML = '<div style="padding:12px;text-align:center;color:rgba(255,255,255,.2);font-size:11px;">No assets yet.<br>Upload files above.</div>';
    return;
  }
  
  var html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">';
  items.slice(0,40).forEach(function(a) {
    var icon = a.type==='image'?'🖼':a.type==='audio'?'🎵':a.type==='sfx'?'🎤':a.type==='video'?'🎬':'📁';
    var thumb = (a.type==='image' && a.url && !a.url.startsWith('local:'))
      ? '<img src="'+a.url+'" style="width:100%;height:44px;object-fit:cover;display:block;">'
      : '<div style="height:44px;background:rgba(255,255,255,.04);display:flex;align-items:center;justify-content:center;font-size:18px;">'+icon+'</div>';
    html += '<div data-asset-id="'+a.id+'" onclick="asset_useInProject(this.dataset.assetId)"'
      + ' style="cursor:pointer;border-radius:6px;overflow:hidden;border:.5px solid rgba(255,255,255,.08);" title="'+a.name+'">'
      + thumb
      + '<div style="font-size:9px;color:rgba(255,255,255,.45);padding:2px 4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+a.name.slice(0,16)+'</div>'
      + '</div>';
  });
  html += '</div>';
  grid.innerHTML = html;
...
// ── 2. asset_upload — actually save file and add to library ─────────────
window.asset_upload = async function(file, type) {
  if (!file) return null;
  type = type || 'image';
  var icon = type==='image'?'🖼':type==='audio'?'🎵':type==='sfx'?'🎤':'🎬';
  
  toast(icon + ' Saving ' + file.name.slice(0,20) + '...', 'info');
  ...
  return new Promise(function(resolve) {
    ...
    reader.onload = async function(e) {
      ...
      var url = dataUrl; // local URL as immediate fallback
      
      // Try to get a permanent URL
      var token = typeof getAuthToken === 'function' ? getAuthToken() : null;
      if (token && type === 'image') {
        try {
          var r = await fetch((window.API||API) + '/api/assets/upload-image', {
            method:'POST',
            headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},
            body:JSON.stringify({ imageBase64:b64, name:file.name, mimeType:file.type })
          });
          var d = await r.json();
          if (d.url) url = d.url; // permanent Imgur URL
        } catch(e) { console.warn('Upload to Imgur failed:', e.message); }
      }
      
      var asset = {
        id: 'asset_'+Date.now(), name:file.name, type, url,
        dataUrl, mimeType:file.type, size:file.size, savedAt:Date.now()
      };
      
      // Save to localStorage
      try {
        var items = JSON.parse(localStorage.getItem('aab_assets')||'[]');
        items.unshift(asset);
        if (items.length > 200) items = items.slice(0,200);
        localStorage.setItem('aab_assets', JSON.stringify(items));
      } catch(e) {}
      
      // If ASSET_LIBRARY exists update it
      if (window.ASSET_LIBRARY) {
        ASSET_LIBRARY.items = ASSET_LIBRARY.items || [];
        ASSET_LIBRARY.items.unshift(asset);
      }
      
      // Also save to DB
      if (token) {
        try {
          await fetch((window.API||API) + '/api/assets', {
            method:'POST',
            headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},
            body:JSON.stringify({ name:file.name, type, url, mime_type:file.type, size_bytes:file.size })
          });
        } catch(e) {}
      }
      
      // Refresh the My Assets grid
      renderMyAssetsGrid();
      
      toast(icon + ' ' + file.name.slice(0,20) + ' saved to Asset Library ✓', 'ok');
      resolve(asset);
    };
    reader.readAsDataURL(file);
  });
...
// ── 3. aps_goToEditSuite — show button, load AI clips ───────────────
window.aps_goToEditSuite = function() {
  // Collect ALL generated clips from APS_CLIPS and PROJECT.clips
  var clips = {};
  
  // From APS_CLIPS (runtime)
  try {
    if (window.APS_CLIPS) Object.assign(clips, APS_CLIPS);
  } catch(e) {}
  ...
  // From PROJECT.clips (current project)
  try {
    if (PROJECT && PROJECT.clips) Object.assign(clips, PROJECT.clips);
  } catch(e) {}

  var clipList = Object.values(clips).filter(function(c) { return c && (c.url || c.videoUrl); });
  
  if (!clipList.length) {
    toast('No generated clips yet — generate at least one scene first', 'info');
    return;
  }

  // Add all clips to PROJECT.clips for the edit suite
  if (!PROJECT.clips) PROJECT.clips = {};
  clipList.forEach(function(c) {
    var key = c.sceneId || ('aps_' + (c.sceneNum || Date.now()));
    PROJECT.clips[key] = {
      sceneId:   c.sceneId || key,
      sceneNum:  c.sceneNum || 0,
      sceneName: c.sceneName || ('AI Scene ' + (c.sceneNum || '')),
      url:       c.url || c.videoUrl,
      videoUrl:  c.url || c.videoUrl,
      duration:  c.duration || 8,
      source:    'ai-presenter',
      provider:  c.provider || APS.settings.provider || 'did',
      addedAt:   Date.now()
    };
  });
  ...
  try {
    var lsClips = JSON.parse(localStorage.getItem('aab_aps_clips') || '{}');
    Object.assign(lsClips, PROJECT.clips);
    localStorage.setItem('aab_aps_clips', JSON.stringify(lsClips));
  } catch(e) {}

  // Build the edit suite timeline from these clips
  if (typeof tl_buildTracksFromProject === 'function') {
    setTimeout(tl_buildTracksFromProject, 100);
  }
  if (typeof edit_buildPlaylist === 'function') {
    setTimeout(edit_buildPlaylist, 150);
  }
  if (typeof edit_renderClipStrip === 'function') {
    setTimeout(edit_renderClipStrip, 200);
  }

  // Navigate to edit suite
  nav('edit-pg');
  toast('✓ ' + clipList.length + ' clip' + (clipList.length !== 1 ? 's' : '') + ' transferred to Edit Suite', 'ok');
};;

// ── 4. Show "Go to Edit Suite" button after batch completes ─────────
// Patch the batch success handler to also show the button
var _patchBatchOnSuccess = window.aps_onBatchSuccess;
window.aps_showGoToEditBtn = function(clipCount) {
  var btn = document.getElementById('aps-go-edit-btn');
  var empty = document.getElementById('aps-clips-empty');
  if (btn) {
    btn.style.display = 'block';
    btn.textContent = '🎬 Go to Edit Suite (' + clipCount + ' clips) →';
  }
  if (empty) empty.style.display = 'none';
  // Enable topbar button immediately when first clip is ready
  var topBtn = document.getElementById('aps-go-edit-btn-top');
  if (topBtn) {
    topBtn.disabled = false;
    topBtn.style.cssText = 'background:#10b981;color:#fff;border:1px solid #10b981;padding:7px 16px;border-radius:6px;cursor:pointer;font-size:12px;font-weight:700;transition:all .2s;box-shadow:0 0 14px rgba(16,185,129,.45);';
    topBtn.textContent = '🎬 Go to Edit Suite (' + clipCount + ') →';
    topBtn.title = 'Transfer all ' + clipCount + ' clip' + (clipCount !== 1 ? 's' : '') + ' to Edit Suite timeline';
  }
};

// Intercept every aps_addClipToBar call to show the Go button
var _superBase = window.aps_addClipToBar;
window.aps_addClipToBar = function(sc) {
  if (typeof _superBase === 'function') _superBase(sc);
  var count = Object.keys(window.APS_CLIPS||{}).length;
  window.aps_showGoToEditBtn(count || 1);
};

// ── 5. importToTimeline — complete rewrite to actually work ─────────
window.importToTimeline = function(inputOrFile, type) {
  var file;
  if (inputOrFile && inputOrFile.files) file = inputOrFile.files[0];
  else if (inputOrFile instanceof File) file = inputOrFile;
  if (!file) return;
  
  type = type || (file.type.startsWith('video') ? 'video' : file.type.startsWith('audio') ? 'audio' : 'image');
  var icon = type==='video'?'🎬':type==='audio'?'🎵':type==='sfx'?'🎤':'🖼';
  var url = URL.createObjectURL(file);
  
  toast(icon + ' ' + file.name.slice(0,20) + ' loading...', 'info');
  
  // Add to timeline immediately
  if (!window.TL) window.TL = { tracks:[], duration:0, zoom:40, playhead:0 };
  if (!TL.tracks) TL.tracks = [];
  
  var trackMap = { video:'broll', audio:'music', sfx:'sfx', image:'overlays' };
  var nameMap  = { video:'🎞 B-Roll', audio:'🎵 Music', sfx:'🎤 SFX', image:'🖼 Images' };
  var trackId  = trackMap[type] || 'broll';
  
  var track = TL.tracks.find(function(t){ return t.id===trackId; });
  if (!track) {
    track = { id:trackId, type:type==='sfx'?'audio':type, name:nameMap[type]||'Track', muted:false, volume:1, clips:[] };
    TL.tracks.push(track);
  }
  
  var maxEnd = (track.clips||[]).reduce(function(m,c){ return Math.max(m, c.start+(c.duration||8)); }, TL.playhead||0);
  track.clips = track.clips || [];
  track.clips.push({
    id:       'import_'+Date.now(),
    url,
    label:    icon + ' ' + file.name.slice(0,18),
    start:    maxEnd,
    duration: type==='audio'||type==='sfx' ? 30 : 8,
    type,
    volume:   1,
    muted:    false,
  });
  ...
  try {
    var items = JSON.parse(localStorage.getItem('aab_assets')||'[]');
    items.unshift({ id:'import_'+Date.now(), name:file.name, type, url, mimeType:file.type, size:file.size, savedAt:Date.now() });
    if (items.length > 200) items = items.slice(0,200);
    localStorage.setItem('aab_assets', JSON.stringify(items));
  } catch(e) {}
  
  if (typeof tl_render === 'function') tl_render();
  toast(icon + ' ' + file.name.slice(0,20) + ' added to timeline ✓', 'ok');
  
  // Also upload permanently in background
  var token = typeof getAuthToken === 'function' ? getAuthToken() : null;
  if (token && type === 'image') {
    var reader = new FileReader();
    reader.onload = async function(e) {
      ...
      try {
        ...
        var d = await r.json();
        if (d.url) {
          // Update the clip URL to permanent one
          track.clips.forEach(function(c){ if(c.url===url) c.url=d.url; });
          if (typeof tl_render === 'function') tl_render();
        }
      } catch(e) {}
    };
...
// ── 6. HeyGen Enterprise notice — shown when photo upload fails ──────
window.aps_showPlanNotice = function(msg) {
  var notice = document.getElementById('aps-plan-notice');
  if (!notice) {
    notice = document.createElement('div');
    notice.id = 'aps-plan-notice';
    notice.style.cssText = 'background:rgba(245,158,11,.12);border:.5px solid rgba(245,158,11,.3);border-radius:8px;padding:10px 12px;font-size:11px;color:rgba(245,158,11,.9);margin:8px 0;line-height:1.5;';
    var sidebar = document.querySelector('.aps-sidebar, #aps-right-panel, [id*="aps"][id*="sidebar"]');
    if (sidebar) sidebar.insertBefore(notice, sidebar.firstChild);
  }
  notice.innerHTML = '⚠ <strong>HeyGen Plan Note:</strong><br>' + msg + '<br><a href="https://www.heygen.com/pricing" target="_blank" style="color:#f59e0b;text-decoration:underline;">Upgrade on HeyGen ↗</a>';
};

// Patch photo upload to show notice if custom face fails
var _origLoadPhoto = window.aps_loadPhoto;
window.aps_loadPhoto = function(input) {
  ...
  reader.onload = function(e) {
    ...
    APS.talkingPhotoId   = null;
    APS.photoPreviewUrl  = null;

    // Persist immediately
    try { localStorage.setItem('aps_presenter_photo', dataUrl); } catch(e) {}

    // Update large presenter preview (centre canvas)
    var wrap = document.getElementById('aps-presenter-img-wrap');
    if (wrap) wrap.innerHTML = '<img src="' + dataUrl + '" style="width:100%;height:100%;object-fit:cover;object-position:top;border-radius:100px 100px 0 0;">';

    // Update small circular preview in right panel
    var prevWrap = document.getElementById('aps-photo-preview-wrap');
    var prev     = document.getElementById('aps-photo-preview');
    var nameEl   = document.getElementById('aps-photo-name');
    var labelTxt = document.getElementById('aps-upload-label-text');
    if (prev)     { prev.src = dataUrl; }
    if (prevWrap) { prevWrap.style.display = 'flex'; }
    ...
    // Auto-switch provider to D-ID
    var provSel = document.getElementById('aps-provider');
    if (provSel && provSel.value !== 'did') {
      provSel.value = 'did';
      if (APS.settings) APS.settings.provider = 'did';
      if (typeof aps_onProviderChange === 'function') aps_onProviderChange('did');
    }

    // Auto-save to current character if name is set
    var charName = APS.charName || (document.getElementById('aps-char-name') || {}).value;
    if (charName && charName.trim()) {
      try {
        var chars = JSON.parse(localStorage.getItem('aab_characters') || '{}');
        if (chars[charName.trim()]) {
          chars[charName.trim()].photoB64 = b64;
          chars[charName.trim()].photoMimeType = file.type || 'image/jpeg';
          localStorage.setItem('aab_characters', JSON.stringify(chars));
          if (typeof aps_renderCharGrid === 'function') setTimeout(aps_renderCharGrid, 100);
        }
      } catch(e) {}
    }
  };
...
// ── 7. Recording studio → Edit Suite path stays separate ────────────
// When user records a video clip, it saves to REC_CLIPS
// Navigating to Edit Suite from recording loads REC_CLIPS, not APS_CLIPS
document.addEventListener('aab_nav', function(e) {
  var pg = e.detail && e.detail.page;
  if (pg === 'edit-pg') {
    if (PROJECT._editMode === 'ai') {
      // Coming from APS — already loaded in aps_goToEditSuite
    } else {
      // Coming from Recording Studio — restore recording clips
      PROJECT._editMode = 'recording';
      // Don't overwrite if already has clips from recording
      if (Object.keys(PROJECT.clips||{}).length === 0) {
        // Try to restore from localStorage
        try {
          var saved = JSON.parse(localStorage.getItem('aab_recording_clips') || '{}');
          if (Object.keys(saved).length) PROJECT.clips = saved;
        } catch(e) {}
      }
    }
    setTimeout(function() {
  ...
  if (pg === 'ai-presenter-studio') {
    PROJECT._editMode = 'ai';
  }
});

// Patch updateStudioClips to save to REC_CLIPS
var _baseUpdateStudioClips = window.updateStudioClips;
window.updateStudioClips = function() {
  if (typeof _baseUpdateStudioClips === 'function') _baseUpdateStudioClips();
  // Copy recording clips to REC_CLIPS and persist
  Object.entries(PROJECT.clips||{}).forEach(function([k,v]) {
    if (v.source === 'recording') window.REC_CLIPS[k] = v;
  });
  try { localStorage.setItem('aab_recording_clips', JSON.stringify(window.REC_CLIPS)); } catch(e) {}
};

// ── 8. Timeline scroll CSS — ensure full width visible ───────────────
var existingStyle = document.getElementById('aab-tl-scroll-fix');
if (existingStyle) existingStyle.remove();
var tlStyle = document.createElement('style');
tlStyle.id = 'aab-tl-scroll-fix';
tlStyle.textContent = [
  '#tl-scroll-container { overflow-x:auto !important; overflow-y:auto !important; }',
  '#tl-tracks { min-width:max-content !important; }',
  '#tl-ruler { min-width:max-content !important; }',
  '.tl-track-rail { min-width:600px; }',
  '#edit-pg .edit-main { min-height:0 !important; }',
].join('');
document.head.appendChild(tlStyle);



// ── EDIT SUITE LAYOUT FIX — clips strip vertical, timeline bottom ─
// Override edit_renderClipStrip to render vertically (clips listed top-to-bottom)
var _origRenderStrip = window.edit_renderClipStrip;
window.edit_renderClipStrip = function() {
  var inner = document.getElementById('edit-clips-strip-inner');
  if (!inner) { if (typeof _origRenderStrip === 'function') return _origRenderStrip(); return; }

  var clips = Object.values(PROJECT.clips || {});
  // Sort chronologically by sceneNum
  clips.sort(function(a,b){ return (a.sceneNum||a.sceneIdx||0) - (b.sceneNum||b.sceneIdx||0); });

  var countEl = document.getElementById('edit-clip-count');
  if (countEl) countEl.textContent = clips.length + ' clip' + (clips.length !== 1 ? 's' : '');

  if (!clips.length) {
    inner.innerHTML = '<div style="font-size:11px;color:rgba(255,255,255,.2);text-align:center;padding:20px 0;">No clips yet<br><small style="font-size:9px;opacity:.6;">Record scenes or generate with AI</small></div>';
    return;
  }
  ...
  clips.forEach(function(clip, idx) {
    ...
    var isSelected = (typeof EDIT_PLAYLIST !== 'undefined' && EDIT_PLAYLIST.current === idx);
    var sourceIcon = clip.source === 'ai-presenter' ? '✨' : '🎥';
    var duration   = clip.duration ? clip.duration.toFixed(1) + 's' : '';

    var card = document.createElement('div');
    card.style.cssText = 'display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:8px;cursor:pointer;border:.5px solid '
      + (isSelected ? 'rgba(0,212,255,.4)' : 'rgba(255,255,255,.06)')
      + ';background:' + (isSelected ? 'rgba(0,212,255,.08)' : 'rgba(255,255,255,.02)')
      + ';transition:background .1s;flex-shrink:0;';

    // Thumbnail
    var thumb = document.createElement('video');
    thumb.src = url;
    thumb.style.cssText = 'width:80px;height:50px;object-fit:cover;border-radius:5px;flex-shrink:0;background:#000;';
    thumb.muted = true;
    thumb.preload = 'metadata';
    thumb.onloadedmetadata = function(){ thumb.currentTime = 0.5; };
    card.appendChild(thumb);

    // Info
    var info = document.createElement('div');
    info.style.cssText = 'flex:1;min-width:0;';
    info.innerHTML = '<div style="font-size:11px;font-weight:600;color:rgba(255,255,255,.8);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'
      + sourceIcon + ' Scene ' + (clip.sceneNum || idx+1) + '</div>'
      + '<div style="font-size:9px;color:rgba(255,255,255,.35);margin-top:2px;">' + (clip.sceneName||'').slice(0,30) + '</div>'
      + '<div style="font-size:9px;color:rgba(255,255,255,.25);margin-top:1px;">' + duration + '</div>';
    card.appendChild(info);

    card.onclick = function() { edit_selectClip(idx); };
    card.onmouseenter = function(){ card.style.background = 'rgba(255,255,255,.05)'; };
    card.onmouseleave = function(){ card.style.background = isSelected ? 'rgba(0,212,255,.08)' : 'rgba(255,255,255,.02)'; };

    inner.appendChild(card);
  });
};

// Also fix edit_buildPlaylist to use the new vertical strip renderer
var _origBuildPlaylist = window.edit_buildPlaylist;
window.edit_buildPlaylist = function() {
  if (typeof _origBuildPlaylist === 'function') _origBuildPlaylist();
  // Re-render the vertical strip after playlist is built
  setTimeout(function() {
    if (typeof edit_renderClipStrip === 'function') edit_renderClipStrip();
  }, 50);
};
...
// Add edit suite CSS
(function() {
  var s = document.getElementById('edit-suite-layout-fix');
  if (s) s.remove();
  s = document.createElement('style');
  s.id = 'edit-suite-layout-fix';
  s.textContent = [
    '#edit-pg { display:flex; flex-direction:column; height:100vh; overflow:hidden; }',
    '#edit-pg > div:not(.edit-topbar-v2) { flex:1; min-height:0; }',
    '#timeline-panel { flex-shrink:0 !important; height:220px !important; }',
    '#tl-scroll-container { overflow-x:auto !important; overflow-y:auto !important; }',
    '#tl-tracks { min-width:max-content; }',
    '.tl-clip { cursor:pointer; border-radius:4px; }',
    '.tl-clip:hover { opacity:.85; }',
    '.tl-clip.selected { outline:2px solid var(--accent); }',
    /* clips strip vertical scroll */
    '#edit-clips-strip-inner::-webkit-scrollbar { width:4px; }',
    '#edit-clips-strip-inner::-webkit-scrollbar-thumb { background:rgba(255,255,255,.15); border-radius:2px; }',
  ].join('\n');
  document.head.appendChild(s);
})();
...
// ═══════════════════════════════════════════════════════════════
// DASHBOARD — My Videos + My Assets full implementation
// ═══════════════════════════════════════════════════════════════

// dashNav — handle all dashboard sections including new ones
var _origDashNav = window.dashNav;
window.dashNav = function(section, btn) {
  ...
  var panel = document.getElementById('dash-' + section);
  if (panel) panel.classList.add('on');

  // Load data for the section
  if (section === 'my-videos')  dash_loadVideos();
  if (section === 'my-assets')  dash_loadAssets();
  if (section === 'projects' && typeof _origDashNav === 'function') {
    // Let original handle projects loading
    _origDashNav('projects', btn);
    panel && panel.classList.add('on'); // re-add since origDashNav hides it
  }
};

// ── My Videos ──────────────────────────────────────────────────
window._allVideos = [];
window._videoFilter = 'all';

window.dash_loadVideos = function() {
  var grid = document.getElementById('dash-videos-grid');
  if (!grid) return;
  grid.innerHTML = '<div style="color:rgba(255,255,255,.3);font-size:13px;grid-column:1/-1;padding:24px 0;text-align:center;"><div class="spin spin-dk" style="display:inline-block;margin-right:8px;"></div>Loading videos...</div>';

  // Collect from all sources: PROJECT.clips, localStorage APS clips, recording clips
  var videos = [];

  // 1. Current project clips
  Object.values(PROJECT.clips || {}).forEach(function(c) {
    if (c.url || c.videoUrl) {
      videos.push({
        id: c.sceneId || ('clip_' + Date.now() + Math.random()),
        url: c.url || c.videoUrl,
        name: c.sceneName || ('Scene ' + (c.sceneNum || '')),
        source: c.source || 'recording',
        duration: c.duration || 0,
        sceneNum: c.sceneNum || 0,
        savedAt: c.ts || c.savedAt || 0,
      });
    }
  });

  // 2. Saved APS (AI) clips from localStorage
  try {
    var apsClips = JSON.parse(localStorage.getItem('aab_aps_clips') || '{}');
    Object.values(apsClips).forEach(function(c) {
      if ((c.url || c.videoUrl) && !videos.find(function(v){ return v.url === (c.url||c.videoUrl); })) {
        videos.push({
          id: c.sceneId || ('aps_'+Date.now()+Math.random()),
          url: c.url || c.videoUrl,
          name: c.sceneName || 'AI Scene',
          source: 'ai-presenter',
          duration: c.duration || 0,
          sceneNum: c.sceneNum || 0,
          savedAt: c.savedAt || 0,
  ...
  // 3. Recording clips from localStorage
  try {
    var recClips = JSON.parse(localStorage.getItem('aab_recording_clips') || '{}');
    Object.values(recClips).forEach(function(c) {
      if ((c.url || c.videoUrl) && !videos.find(function(v){ return v.url === (c.url||c.videoUrl); })) {
        videos.push({
          id: c.sceneId || ('rec_'+Date.now()+Math.random()),
          url: c.url || c.videoUrl,
          name: c.sceneName || ('Scene ' + (c.sceneNum || '')),
          source: 'recording',
          duration: c.duration || 0,
  ...
  // Sort by scene number then recency
  videos.sort(function(a,b){ return (b.savedAt||0) - (a.savedAt||0); });
  window._allVideos = videos;
  dash_renderVideos(videos);
};

window.dash_filterVideos = function(filter, btn) {
  window._videoFilter = filter;
  document.querySelectorAll('.dash-vid-filter').forEach(function(b){ b.classList.remove('on'); });
  if (btn) btn.classList.add('on');
  var filtered = filter === 'all' ? window._allVideos
    : window._allVideos.filter(function(v){ return v.source === filter; });
  dash_renderVideos(filtered);
};

window.dash_renderVideos = function(videos) {
  var grid = document.getElementById('dash-videos-grid');
  if (!grid) return;
  if (!videos.length) {
    grid.innerHTML = '<div style="color:rgba(255,255,255,.3);font-size:13px;grid-column:1/-1;padding:40px 0;text-align:center;">No videos yet.<br><small style="opacity:.5;">Record scenes or generate with AI Presenter.</small></div>';
    return;
  }
  grid.innerHTML = '';
  videos.forEach(function(v) {
    var icon = v.source === 'ai-presenter' ? '✨' : '🎥';
    var dur  = v.duration ? v.duration.toFixed(1) + 's' : '';
    var card = document.createElement('div');
    card.style.cssText = 'background:rgba(255,255,255,.03);border:.5px solid rgba(255,255,255,.08);border-radius:10px;overflow:hidden;cursor:pointer;transition:border-color .15s;';
    card.innerHTML =
      '<video src="'+v.url+'" style="width:100%;height:120px;object-fit:cover;display:block;background:#000;" muted preload="metadata" onloadedmetadata="this.currentTime=0.5"></video>'
      + '<div style="padding:8px 10px;">'
      + '<div style="font-size:11px;font-weight:600;color:rgba(255,255,255,.85);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+icon+' '+v.name+'</div>'
      + (dur ? '<div style="font-size:9px;color:rgba(255,255,255,.3);margin-top:2px;">'+dur+'</div>' : '')
      + '<div style="display:flex;gap:6px;margin-top:8px;">'
      + '<button onclick="event.stopPropagation();window._dashEditId=\''+v.id+'\';dash_loadIntoEdit(window._dashEditId)" style="flex:1;font-size:9px;padding:3px 0;background:rgba(0,212,255,.12);border:.5px solid rgba(0,212,255,.3);color:rgba(0,212,255,.8);border-radius:5px;cursor:pointer;">Edit →</button>'
      + '<a href="'+v.url+'" download style="flex:1;font-size:9px;padding:3px 0;background:rgba(255,255,255,.05);border:.5px solid rgba(255,255,255,.1);color:rgba(255,255,255,.5);border-radius:5px;cursor:pointer;text-align:center;text-decoration:none;">↓ Save</a>'
      + '</div>'
      + '</div>';
    card.onmouseenter = function(){ card.style.borderColor = 'rgba(0,212,255,.3)'; };
    card.onmouseleave = function(){ card.style.borderColor = 'rgba(255,255,255,.08)'; };
    card.onclick = function() {
      // Open in edit suite with this clip loaded
      var vid = document.getElementById('edit-video');
      if (vid) { vid.src = v.url; vid.style.display = 'block'; vid.load(); }
      var noClips = document.getElementById('edit-no-clips');
      if (noClips) noClips.style.display = 'none';
      nav('edit-pg');
    };
...
window.dash_loadIntoEdit = function(id) {
  var v = window._allVideos.find(function(x){ return x.id === id; });
  if (!v) return;
  var vid = document.getElementById('edit-video');
  if (vid) { vid.src = v.url; vid.style.display = 'block'; vid.load(); }
  nav('edit-pg');
...
// ── My Assets ──────────────────────────────────────────────────
window._allAssets = [];
window._assetFilter = 'all';

window.dash_loadAssets = function() {
  var grid = document.getElementById('dash-assets-grid');
  if (!grid) return;

  var assets = [];
  try {
    assets = JSON.parse(localStorage.getItem('aab_assets') || '[]');
  } catch(e) {}

  // Also include presenter photo if saved
  var presPhoto = localStorage.getItem('aps_presenter_photo');
  if (presPhoto && !assets.find(function(a){ return a.name === 'Presenter Photo'; })) {
    assets.unshift({ id: 'pres_photo', name: 'Presenter Photo', type: 'image', url: 'data:image/jpeg;base64,' + presPhoto.slice(0, 50000), savedAt: Date.now() });
  }

  window._allAssets = assets;
  dash_renderAssets(assets);
};

window.dash_filterAssets = function(type, btn) {
  window._assetFilter = type;
  document.querySelectorAll('.dash-asset-filter').forEach(function(b){ b.classList.remove('on'); });
  if (btn) btn.classList.add('on');
  var filtered = type === 'all' ? window._allAssets
    : window._allAssets.filter(function(a){ return a.type === type; });
  dash_renderAssets(filtered);
};

window.dash_renderAssets = function(assets) {
  var grid = document.getElementById('dash-assets-grid');
  if (!grid) return;
  if (!assets.length) {
    grid.innerHTML = '<div style="color:rgba(255,255,255,.3);font-size:13px;grid-column:1/-1;padding:40px 0;text-align:center;">No assets yet.<br><small style="opacity:.5;">Upload images, audio, video or SFX above.</small></div>';
    return;
  }
  grid.innerHTML = '';
  assets.forEach(function(a) {
    var icon = a.type==='image'?'🖼':a.type==='audio'?'🎵':a.type==='video'?'🎬':a.type==='sfx'?'🎤':'📁';
    var thumb = (a.type==='image' && a.url && (a.url.startsWith('http') || a.url.startsWith('data:')))
      ? '<img src="'+a.url+'" style="width:100%;height:90px;object-fit:cover;display:block;background:#111;" loading="lazy">'
      : '<div style="height:90px;display:flex;align-items:center;justify-content:center;font-size:28px;background:rgba(255,255,255,.03);">'+icon+'</div>';
    var card = document.createElement('div');
    card.style.cssText = 'background:rgba(255,255,255,.03);border:.5px solid rgba(255,255,255,.08);border-radius:8px;overflow:hidden;';
    card.innerHTML = thumb
      + '<div style="padding:6px 8px;">'
      + '<div style="font-size:10px;font-weight:600;color:rgba(255,255,255,.7);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="'+a.name+'">'+a.name+'</div>'
      + '<div style="font-size:9px;color:rgba(255,255,255,.3);margin-top:1px;text-transform:uppercase;">'+a.type+'</div>'
      + '</div>';
    grid.appendChild(card);
  });
};

window.dash_uploadAsset = function(file, type) {
  if (!file) return;
  if (typeof asset_upload === 'function') {
    asset_upload(file, type).then(function() {
      dash_loadAssets();
      toast('✓ ' + file.name.slice(0,25) + ' saved to My Assets', 'ok');
    });
  } else {
    ...
    reader.onload = function(e) {
      var items = [];
      try { items = JSON.parse(localStorage.getItem('aab_assets') || '[]'); } catch(x) {}
      items.unshift({ id: 'a_'+Date.now(), name: file.name, type: type, url: e.target.result, savedAt: Date.now() });
      try { localStorage.setItem('aab_assets', JSON.stringify(items.slice(0,200))); } catch(x) {}
      dash_loadAssets();
      toast('✓ ' + file.name.slice(0,25) + ' saved', 'ok');
    };
    reader.readAsDataURL(file);
  }
};

// Add "My Videos" and "My Assets" buttons to dashboard sidebar if not already there
(function addDashButtons() {
  var nav = document.querySelector('.dash-sidebar .sb-nav, #dashboard-pg .sb-nav');
  if (!nav) return;
  if (nav.querySelector('[onclick*="my-videos"]')) return; // already added

  var divider = nav.querySelector('.sb-div');
  if (!divider) return;

  // Insert before the divider before Account
  var accountBtn = nav.querySelector('[onclick*="account"]');
  if (!accountBtn) return;

  var myVideosBtn = document.createElement('button');
  myVideosBtn.className = 'sb-item';
  myVideosBtn.innerHTML = '<svg width="13" height="13" viewBox="0 0 14 14" fill="none"><rect x="1" y="2" width="12" height="10" rx="2" stroke="currentColor" stroke-width="1.2"/><path d="M6 5l4 2-4 2V5z" fill="currentColor"/></svg>My Videos';
  myVideosBtn.onclick = function() { dashNav('my-videos', this); };

  var myAssetsBtn = document.createElement('button');
  myAssetsBtn.className = 'sb-item';
  myAssetsBtn.innerHTML = '<svg width="13" height="13" viewBox="0 0 14 14" fill="none"><rect x="1" y="1" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/><rect x="7" y="1" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/><rect x="1" y="7" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/><rect x="7" y="7" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.2"/></svg>My Assets';
  myAssetsBtn.onclick = function() { dashNav('my-assets', this); };

  var sep = document.createElement('div');
  sep.className = 'sb-div';

  // Add before account button's preceding divider
  accountBtn.parentElement.insertBefore(myVideosBtn, accountBtn);
  accountBtn.parentElement.insertBefore(myAssetsBtn, accountBtn);
  accountBtn.parentElement.insertBefore(sep, accountBtn);
})();

// Add My Videos + My Assets panels to dash-main if not there
(function addDashPanels() {
  var dashMain = document.querySelector('.dash-main');
  if (!dashMain || document.getElementById('dash-my-videos')) return;

  var videosPanel = document.createElement('div');
  videosPanel.className = 'dash-sec';
  videosPanel.id = 'dash-my-videos';
  videosPanel.innerHTML = '<div class="dash-hdr"><div><div class="dash-title">My Videos</div><div class="dash-sub">All your recorded and AI-generated clips</div></div>'
    + '<div style="display:flex;gap:8px;"><button class="btn btn-ghost btn-sm" onclick="dash_loadVideos()">↻ Refresh</button>'
    + '<button class="btn btn-primary btn-sm" onclick="nav(&apos;edit-pg&apos;)">Open Edit Suite →</button></div></div>'
    + '<div style="display:flex;gap:6px;margin-bottom:16px;">'
    + '<button class="btn btn-ghost btn-sm dash-vid-filter on" onclick="dash_filterVideos(&apos;all&apos;,this)">All</button>'
    + '<button class="btn btn-ghost btn-sm dash-vid-filter" onclick="dash_filterVideos(&apos;recording&apos;,this)">🎥 Recorded</button>'
    + '<button class="btn btn-ghost btn-sm dash-vid-filter" onclick="dash_filterVideos(&apos;ai-presenter&apos;,this)">✨ AI Generated</button>'
    + '</div>'
    + '<div id="dash-videos-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px;">'
    + '<div style="color:rgba(255,255,255,.3);font-size:13px;grid-column:1/-1;padding:24px 0;text-align:center;">Click Refresh to load your videos</div>'
    + '</div>';

  var assetsPanel = document.createElement('div');
  assetsPanel.className = 'dash-sec';
  assetsPanel.id = 'dash-my-assets';
  assetsPanel.innerHTML = '<div class="dash-hdr"><div><div class="dash-title">My Assets</div><div class="dash-sub">Images, audio, video and SFX</div></div></div>'
    + '<div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;">'
    + '<label class="btn btn-ghost btn-sm" style="cursor:pointer;">🖼 Image<input type="file" style="display:none;" accept="image/*" onchange="dash_uploadAsset(this.files[0],&apos;image&apos;)"></label>'
    + '<label class="btn btn-ghost btn-sm" style="cursor:pointer;">🎵 Audio<input type="file" style="display:none;" accept="audio/*" onchange="dash_uploadAsset(this.files[0],&apos;audio&apos;)"></label>'
    + '<label class="btn btn-ghost btn-sm" style="cursor:pointer;">🎬 Video<input type="file" style="display:none;" accept="video/*" onchange="dash_uploadAsset(this.files[0],&apos;video&apos;)"></label>'
    + '<label class="btn btn-ghost btn-sm" style="cursor:pointer;">🎤 SFX<input type="file" style="display:none;" accept="audio/*" onchange="dash_uploadAsset(this.files[0],&apos;sfx&apos;)"></label>'
    + '</div>'
    + '<div style="display:flex;gap:6px;margin-bottom:14px;">'
    + '<button class="btn btn-ghost btn-sm dash-asset-filter on" onclick="dash_filterAssets(&apos;all&apos;,this)">All</button>'
    + '<button class="btn btn-ghost btn-sm dash-asset-filter" onclick="dash_filterAssets(&apos;image&apos;,this)">🖼 Images</button>'
    + '<button class="btn btn-ghost btn-sm dash-asset-filter" onclick="dash_filterAssets(&apos;audio&apos;,this)">🎵 Audio</button>'
    + '<button class="btn btn-ghost btn-sm dash-asset-filter" onclick="dash_filterAssets(&apos;video&apos;,this)">🎬 Video</button>'
    + '<button class="btn btn-ghost btn-sm dash-asset-filter" onclick="dash_filterAssets(&apos;sfx&apos;,this)">🎤 SFX</button>'
    + '</div>'
    + '<div id="dash-assets-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px;">'
    + '<div style="color:rgba(255,255,255,.3);font-size:13px;grid-column:1/-1;padding:24px 0;text-align:center;">No assets yet — upload above</div>'
    + '</div>';

  // Insert before dash-account
  var accountSec = document.getElementById('dash-account');
  if (accountSec) {
    dashMain.insertBefore(videosPanel, accountSec);
    dashMain.insertBefore(assetsPanel, accountSec);
  } else {
    dashMain.appendChild(videosPanel);
    dashMain.appendChild(assetsPanel);
  }
})();
...
window.aps_onProviderChange = function(val) {
  var note = document.getElementById('aps-provider-note');
  if (!note) return;
  var msgs = {
    'did':    '⚡ D-ID: animates your photo in 30-90s/scene. Requires photo upload.',
    'heygen': '⏱ HeyGen: stock avatar only on Creator plan (3-8min/scene). Your photo ignored.',
    'kling':  '🎬 Kling: image-to-video motion. No lip-sync. Good for cinematic backgrounds.',
  };
  note.textContent = msgs[val] || '';
  note.style.color = val === 'did' ? 'rgba(0,212,255,.7)' : 'rgba(255,200,0,.6)';
};

// Auto-select D-ID when photo is uploaded
var _origApsLoadPhoto = window.aps_loadPhoto;
window.aps_loadPhoto = function(input) {
  if (typeof _origApsLoadPhoto === 'function') _origApsLoadPhoto.apply(this, arguments);
  // Auto-switch to D-ID when photo uploaded (it's the only provider that uses it reliably)
  setTimeout(function() {
    var sel = document.getElementById('aps-provider');
    if (sel && sel.value !== 'did') {
      sel.value = 'did';
      if (APS && APS.settings) APS.settings.provider = 'did';
      if (typeof aps_onProviderChange === 'function') aps_onProviderChange('did');
      toast('Photo uploaded ✓ — D-ID will animate your face in 30-90s per scene', 'ok');
    }
  }, 500);
};


// ══ CHARACTER GRID — visual card picker for saved characters ══════════════
window.aps_renderCharGrid = function() {
  var grid = document.getElementById('aps-char-grid');
  if (!grid) return;

  var chars = {};
  try { chars = JSON.parse(localStorage.getItem('aab_characters') || '{}'); } catch(e) {}

  if (!Object.keys(chars).length) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;color:rgba(255,255,255,.2);font-size:10px;padding:16px 0;">No saved characters yet — upload a photo, enter a name and click Save</div>';
    return;
  }

  grid.innerHTML = '';
  Object.values(chars).forEach(function(char) {
    var card = document.createElement('div');
    var isActive = APS.charName === char.name;
    card.style.cssText = 'border-radius:8px;overflow:hidden;cursor:pointer;border:1.5px solid ' +
      (isActive ? 'rgba(0,212,255,.6)' : 'rgba(255,255,255,.08)') +
      ';background:' + (isActive ? 'rgba(0,212,255,.08)' : 'rgba(255,255,255,.03)') +
      ';transition:border-color .15s;';

    // Photo or placeholder
    var imgHtml;
    if (char.photoB64) {
      imgHtml = '<img src="data:' + (char.photoMimeType||'image/jpeg') + ';base64,' + char.photoB64 +
        '" style="width:100%;height:52px;object-fit:cover;object-position:top;display:block;" />';
    } else {
      imgHtml = '<div style="width:100%;height:52px;background:rgba(255,255,255,.05);display:flex;align-items:center;justify-content:center;font-size:18px;">👤</div>';
    }

    card.innerHTML = imgHtml +
      '<div style="padding:4px 5px;">' +
      '<div style="font-size:9px;font-weight:600;color:rgba(255,255,255,.8);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + char.name + '">' + char.name + '</div>' +
      '<div style="font-size:8px;color:rgba(255,255,255,.3);">' + (char.voiceName || '') + '</div>' +
      '</div>' +
      (isActive ? '<div style="font-size:8px;text-align:center;color:rgba(0,212,255,.7);padding:2px 0;border-top:.5px solid rgba(0,212,255,.2);">Active</div>' : '');

    card.onclick = function() {
      aps_loadCharacterFromData(char);
      // Update active state
      Array.from(grid.children).forEach(function(c) {
        c.style.borderColor = 'rgba(255,255,255,.08)';
        c.style.background  = 'rgba(255,255,255,.03)';
      });
      card.style.borderColor = 'rgba(0,212,255,.6)';
      card.style.background  = 'rgba(0,212,255,.08)';
    };

    card.onmouseenter = function() {
      if (APS.charName !== char.name) card.style.borderColor = 'rgba(255,255,255,.2)';
    };
    card.onmouseleave = function() {
      if (APS.charName !== char.name) card.style.borderColor = 'rgba(255,255,255,.08)';
    };
    ...
    del.style.cssText = 'position:absolute;top:3px;right:3px;background:rgba(0,0,0,.6);border:none;color:rgba(255,255,255,.5);border-radius:50%;width:14px;height:14px;font-size:10px;cursor:pointer;line-height:1;display:none;';
    del.onclick = function(e) {
      e.stopPropagation();
      if (!confirm('Delete character "' + char.name + '"?')) return;
      var cs = JSON.parse(localStorage.getItem('aab_characters') || '{}');
      delete cs[char.name];
      localStorage.setItem('aab_characters', JSON.stringify(cs));
      aps_renderCharGrid();
...
// Load character from data object (not just name)
window.aps_loadCharacterFromData = function(char) {
  if (!char) return;

  // Restore photo
  if (char.photoB64) {
    APS.photoB64      = char.photoB64;
    APS.photoMimeType = char.photoMimeType || 'image/jpeg';
    var imgWrap = document.getElementById('aps-presenter-img-wrap');
    if (imgWrap) {
      imgWrap.innerHTML = '<img src="data:' + (char.photoMimeType||'image/jpeg') + ';base64,' + char.photoB64 +
        '" style="width:100%;height:100%;object-fit:cover;object-position:top;border-radius:100px 100px 0 0;" />';
    }
    var photoPreview = document.getElementById('aps-photo-preview');
    if (photoPreview) {
      photoPreview.src = 'data:' + (char.photoMimeType||'image/jpeg') + ';base64,' + char.photoB64;
      photoPreview.style.display = 'block';
    }
  }

  // Restore settings
  if (char.talkingPhotoId) APS.talkingPhotoId = char.talkingPhotoId;
  if (APS.settings) {
    if (char.voiceId)   APS.settings.voiceId   = char.voiceId;
    if (char.voiceName) APS.settings.voiceName = char.voiceName;
    if (char.provider)  APS.settings.provider  = char.provider;
    if (char.stability) APS.settings.stability = char.stability;
    if (char.clarity)   APS.settings.clarity   = char.clarity;
  }
  if (char.framing) APS.framing  = char.framing;
  if (char.bgType)  APS.bgType   = char.bgType;
  ...
  // Update voice selector if visible
  var voiceSel = document.getElementById('aps-voice-id');
  if (voiceSel && char.voiceId) voiceSel.value = char.voiceId;

  // Update provider selector
  var provSel = document.getElementById('aps-provider');
  if (provSel && char.provider) {
    provSel.value = char.provider;
    if (typeof aps_onProviderChange === 'function') aps_onProviderChange(char.provider);
  }

  // Save as last used and persist photo for generation
  localStorage.setItem('aab_last_character', char.name);
  if (char.photoB64) {
    // Save to the standard aps_presenter_photo key so generation finds it
    localStorage.setItem('aps_presenter_photo', 'data:' + (char.photoMimeType||'image/jpeg') + ';base64,' + char.photoB64);
  }

  var display = document.getElementById('aps-char-name-display');
  if (display) {
    display.textContent = '✓ Loaded: ' + char.name;
    display.style.color = 'rgba(0,212,255,.7)';
  }

  toast('✓ Character loaded: ' + char.name, 'ok');
};

// Patch aps_saveCharacter to refresh grid after save
var _origSaveChar = window.aps_saveCharacter;
window.aps_saveCharacter = async function(name) {
  if (typeof _origSaveChar === 'function') await _origSaveChar(name);
  // Refresh the visual grid
  setTimeout(aps_renderCharGrid, 100);
};

// Auto-populate grid when APS page is shown
document.addEventListener('aab_nav', function(e) {
  if (e && e.detail && e.detail.page === 'ai-presenter-studio') {
    setTimeout(aps_renderCharGrid, 200);
  }
});

// Also populate on initial load and restore character
setTimeout(function() {
  if (typeof aps_renderCharGrid === 'function') aps_renderCharGrid();
}, 500);
...
console.log('AABStudio final fix ✓ — practice, sync rec countdown, backgrounds, TP bg');
    ...
    window.tl_startResize = function(e) {
      e.preventDefault();
      ...
      function onMove(ev) {
        var delta = startY - ev.clientY; // drag up = larger
        var newH = Math.max(140, Math.min(500, startH + delta));
        panel.style.height = newH + 'px';
      }
      function onUp() {
        document.removeEventListener('mousemove', onMove);
        document.removeEventListener('mouseup', onUp);
        localStorage.setItem('aab_tl_height', panel.offsetHeight);
      }
    ...
    (function() {
      var saved = parseInt(localStorage.getItem('aab_tl_height') || '0');
      if (saved > 140) {
        document.addEventListener('aab_nav', function(e) {
          if (e && e.detail && e.detail.page === 'edit-pg') {
            var panel = document.getElementById('timeline-panel');
            if (panel) panel.style.height = saved + 'px';
          }
        });
      }
    })();
    ...
    window.edit_showInspector = function(clip) {
      var body = document.getElementById('edit-inspector-body');
      if (!body || !clip) return;
      body.innerHTML = [
        '<div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.7);margin-bottom:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + (clip.sceneName||'Clip') + '">' + (clip.sceneName || 'Clip') + '</div>',
        '<div style="font-size:9px;color:rgba(255,255,255,.3);margin-bottom:14px;">' + (clip.provider||'').toUpperCase() + ' · ' + (clip.duration||0) + 's</div>',
        '<div class="insp-label">Volume</div>',
        '<input type="range" min="0" max="1" step="0.05" value="1" style="width:100%;accent-color:#00d4ff;margin-bottom:10px;" oninput="edit_setClipVolume(this.value)">',
        '<div class="insp-label">Speed</div>',
        '<select style="width:100%;background:rgba(255,255,255,.05);border:.5px solid rgba(255,255,255,.1);color:rgba(255,255,255,.7);border-radius:4px;padding:4px;font-size:10px;margin-bottom:10px;" onchange="edit_setSpeed(this.value)">',
        '<option value="0.5">0.5x</option><option value="0.75">0.75x</option><option value="1" selected>1x (normal)</option><option value="1.25">1.25x</option><option value="1.5">1.5x</option><option value="2">2x</option>',
        '</select>',
        '<div class="insp-label" style="margin-bottom:6px;">Clip URL</div>',
        '<div style="font-size:8px;color:rgba(255,255,255,.2);word-break:break-all;background:rgba(255,255,255,.03);padding:4px;border-radius:3px;">' + (clip.url||clip.videoUrl||'').slice(0,60) + '…</div>',
      ].join('');
    };

    window.toggleSafeZones = function() {
      var sz = document.getElementById('edit-safe-zones');
      if (sz) sz.style.display = sz.style.display === 'none' ? 'block' : 'none';
    };

    window.importMedia = function(type) {
      ...
      input.onchange = function() {
        ...
        var clip = { id: 'import_' + Date.now(), sceneName: f.name, url: url, videoUrl: url, duration: 10, source: 'import', provider: type };
        if (!PROJECT.clips) PROJECT.clips = {};
        PROJECT.clips[clip.id] = clip;
        if (typeof aps_addClipToBar === 'function') aps_addClipToBar(clip);
        if (typeof tl_buildTracksFromProject === 'function') tl_buildTracksFromProject();
        toast('✓ ' + f.name + ' imported', 'ok');
      };
      input.click();
    };

    window.edit_skipTo = function(t) {
      var v = document.getElementById('edit-video');
      if (v && isFinite(v.duration)) v.currentTime = Math.min(t, v.duration);
    };

    window.edit_seekTo = function(pct) {
      var v = document.getElementById('edit-video');
      if (v && isFinite(v.duration)) v.currentTime = v.duration * pct / 100;
    };

    // ── CSS for inspector labels ──────────────────────────────────
    (function() {
      var s = document.createElement('style');
      s.textContent = '.insp-label{font-size:9px;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px;}' +
        '.edit-tool-btn.active,.tl-tool.on{background:rgba(0,212,255,.15)!important;color:#00d4ff!important;border-color:rgba(0,212,255,.3)!important;}';
      document.head.appendChild(s);
    })();
